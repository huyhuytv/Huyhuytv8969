<template>
  <div class="game-panel max-w-sm w-full">
    <h3 class="text-accent text-sm mb-3 flex items-center space-x-1">
      <Fish :size="14" />
      <span>{{ $t('fishing_contest.title') }}</span>
    </h3>

    <!-- 准备 -->
    <div v-if="phase === 'ready'">
      <p class="text-xs text-muted mb-3">{{ $t('fishing_contest.desc') }}</p>

      <div v-if="catches.length > 0" class="border border-accent/20 p-2 mb-3">
        <p class="text-xs text-muted mb-1">{{ $t('fishing_contest.catches') }}</p>
        <div
          v-for="(c, i) in catches"
          :key="i"
          class="flex items-center justify-between text-xs py-0.5 border-b border-accent/10 last:border-0"
        >
          <span class="text-accent">{{ $t(`fishing_contest.${c.name}`) }}</span>
          <span class="text-muted">{{ c.weight }}{{ $t('fishing_contest.weight_unit') }} · {{ $t('fishing_contest.score_earned', { score: c.score }) }}</span>
        </div>
        <div class="flex items-center justify-between text-xs mt-1.5 pt-1">
          <span class="text-muted">{{ $t('fishing_contest.current_total') }}</span>
          <span class="text-accent">{{ $t('fishing_contest.points', { score: playerTotal }) }}</span>
        </div>
      </div>

      <p class="text-xs text-muted mb-2">{{ $t('fishing_contest.round', { current: currentRound }) }}</p>
      <Button class="w-full" @click="castLine">{{ $t('fishing_contest.btn_cast') }}</Button>
    </div>
    ...
    <div v-else-if="phase === 'casting'" class="flex flex-col items-center py-8">
      ...
      <p class="text-xs text-muted mt-3">{{ $t('fishing_contest.casting') }}</p>
    </div>
    ...
    <div v-else-if="phase === 'waiting'" class="flex flex-col items-center py-6">
      ...
      <p class="text-xs text-muted">{{ $t('fishing_contest.waiting') }}</p>
    ...
    <div v-else-if="phase === 'tension'">
      <p class="text-xs text-center mb-2 text-accent bite-flash">{{ $t('fishing_contest.bite_hint') }}</p>
      ...
      <div class="flex space-x-3 items-stretch mb-3">
        ...
        <div class="w-8 h-44 bg-bg border border-accent/30 relative overflow-hidden shrink-0">
          ...
          />
          <!-- 标签 -->
          <span class="absolute text-center w-full" style="bottom: 65%; font-size: 8px; color: var(--color-success)">{{ $t('fishing_contest.tension_good') }}</span>
          <span class="absolute text-center w-full" style="bottom: 88%; font-size: 8px; color: var(--color-danger)">{{ $t('fishing_contest.tension_break') }}</span>
        </div>
        ...
        <div class="flex-1 h-44 bg-bg border border-accent/20 relative overflow-hidden">
          ...
          <span class="absolute bottom-0.5 right-1 text-muted" style="font-size: 9px">{{ $t('fishing_contest.round', { current: currentRound }) }}</span>
        </div>
      </div>

      <Button class="w-full py-2.5" :icon="ArrowUp" @click="pullRod">{{ $t('fishing_contest.pull_rod') }}</Button>
      <p class="text-xs text-muted text-center mt-1">{{ $t('fishing_contest.pull_hint') }}</p>
    </div>
    ...
    <div v-else-if="phase === 'round_result'" class="text-center py-4">
      <div :class="resultAnimClass">
        ...
        <div v-if="lastGrade !== 'escaped'" class="mt-2">
          <p class="text-accent text-xs">{{ $t(`fishing_contest.${catches[catches.length - 1]?.name}`) }}</p>
          <p class="text-xs text-muted">{{ catches[catches.length - 1]?.weight }}{{ $t('fishing_contest.weight_unit') }} · {{ $t('fishing_contest.score_earned', { score: catches[catches.length - 1]?.score }) }}</p>
        </div>
        <div v-else class="mt-2">
          <p class="text-xs text-danger">{{ $t('fishing_contest.escaped_desc') }}</p>
        </div>
      </div>
    </div>
    ...
    <!-- 比赛结束 -->
    <div v-else>
      <p class="text-xs text-muted mb-2">{{ $t('fishing_contest.end_title') }}</p>

      <div class="border border-accent/20 p-2 mb-3">
        <p class="text-xs text-muted mb-1">{{ $t('fishing_contest.final_rank') }}</p>
        <div
        ...
        >
          <div>
            <span class="mr-2" :class="{ 'text-accent': i === 0, 'text-success': entry.name === 'you' }">{{ $t('fishing_contest.rank_n', { rank: i + 1 }) }}</span>
            <span :class="{ 'text-success': entry.name === 'you' }">{{ entry.name === 'you' ? $t('harvest_fair.you') : $t(`harvest_fair.${entry.name}`) }}</span>
          </div>
          <span class="text-muted">{{ $t('fishing_contest.points', { score: entry.score }) }}</span>
        </div>
      </div>
      ...
      <div class="mb-3 text-xs text-center border border-accent/20 p-2">
        <span v-if="playerRank === 1" class="text-accent">{{ $t('fishing_contest.rank_1') }}{{ $t('status_bar.money_unit') }}</span>
        <span v-else-if="playerRank === 2" class="text-success">{{ $t('fishing_contest.rank_2') }}{{ $t('status_bar.money_unit') }}</span>
        <span v-else-if="playerRank === 3" class="text-success">{{ $t('fishing_contest.rank_3') }}{{ $t('status_bar.money_unit') }}</span>
        <span v-else class="text-muted">{{ $t('fishing_contest.rank_4') }}</span>
      </div>

      <Button class="w-full" @click="handleClaim">{{ $t('fishing_contest.btn_claim') }}</Button>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const FISH_TIERS = {
    perfect: [
      { name: 'fish_koi', minW: 2.0, maxW: 5.0, baseScore: 50 },
      { name: 'fish_mandarin', minW: 1.5, maxW: 4.0, baseScore: 45 },
      { name: 'fish_sturgeon', minW: 3.0, maxW: 8.0, baseScore: 55 }
    ],
    good: [
      { name: 'fish_bass', minW: 1.5, maxW: 4.0, baseScore: 30 },
      { name: 'fish_catfish', minW: 2.0, maxW: 5.0, baseScore: 25 },
      { name: 'fish_carp', minW: 1.0, maxW: 3.5, baseScore: 20 }
    ],
    poor: [
      { name: 'fish_crucian', minW: 0.5, maxW: 2.0, baseScore: 8 },
      { name: 'fish_grass_carp', minW: 1.0, maxW: 3.0, baseScore: 12 }
    ]
  }
  ...
  const gradeText = computed(() => {
    switch (lastGrade.value) {
      case 'perfect':
        return i18n.global.t('fishing_contest.result_perfect')
      case 'good':
        return i18n.global.t('fishing_contest.result_good')
      case 'poor':
        return i18n.global.t('fishing_contest.result_poor')
      case 'escaped':
        return i18n.global.t('fishing_contest.result_escaped')
    }
  })
  ...
  const finishContest = () => {
    ...
    const npcProfiles: { name: string; perfectRate: number; goodRate: number }[] = [
      { name: 'npc1', perfectRate: 0.55, goodRate: 0.9 },
      { name: 'npc2', perfectRate: 0.45, goodRate: 0.85 },
      { name: 'npc3', perfectRate: 0.35, goodRate: 0.8 }
    ]
    ...
    const player: Participant = { name: 'you', score: playerTotal.value }
