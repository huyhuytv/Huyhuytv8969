<template>
  <div class="game-panel max-w-sm w-full">
    <h3 class="text-accent text-sm mb-3 flex items-center space-x-1">
      <Coffee :size="14" />
      <span>{{ $t('tea_contest.title') }}</span>
    </h3>

    <!-- 准备 -->
    <div v-if="phase === 'ready'">
      <p class="text-xs text-muted mb-3">{{ $t('tea_contest.desc') }}</p>
      <Button class="w-full" @click="startGame">{{ $t('tea_contest.btn_start') }}</Button>
    </div>

    <!-- 泡茶进行中 -->
    <div v-else-if="phase === 'brewing'">
      <div class="flex items-center justify-between mb-2">
        <p class="text-xs text-muted">{{ $t('tea_contest.round_x_of_y', { current: roundIndex + 1, total: 3 }) }}</p>
        <p class="text-xs text-muted">
          {{ $t('tea_contest.total_score') }}
      ...
      <div class="relative h-10 bg-bg border border-accent/20 mb-3">
        ...
        <!-- 目标标签 -->
        <span class="absolute -top-3.5 text-success" style="font-size: 8px" :style="{ left: `calc(${targetPosition}% - 6px)` }">{{ $t('tea_contest.target') }}</span>
        <!-- 填充 -->
        <div
    ...
    <div v-else-if="phase === 'step_result'" class="text-center py-2">
      ...
      <div :class="lastStepGrade === 'perfect' ? 'step-perfect' : lastStepGrade === 'good' ? '' : 'step-miss'">
        ...
        >
          {{ lastStepGrade === 'perfect' ? $t('tea_contest.step_perfect') : lastStepGrade === 'good' ? $t('tea_contest.step_good') : $t('tea_contest.step_poor') }}
          {{ $t('tea_contest.score_earned', { score: lastStepScore }) }}
        </p>
      </div>
    </div>
    ...
    <div v-else-if="phase === 'round_result'" class="text-center py-2">
      ...
      <div :class="lastRoundGrade === 'perfect' ? 'pot-hit' : lastRoundGrade === 'good' ? '' : 'wrong-shake'">
        ...
        >
          {{ lastRoundGrade === 'perfect' ? $t('tea_contest.round_perfect') : lastRoundGrade === 'good' ? $t('tea_contest.round_good') : $t('tea_contest.round_poor') }}
        </p>
        <p class="text-xs text-muted">{{ $t('tea_contest.round_score', { score: roundScore }) }}</p>
      </div>
    </div>

    <!-- 最终结果 -->
    <div v-else-if="phase === 'finished'">
      <p class="text-xs text-muted mb-2">{{ $t('tea_contest.end_title') }}</p>
      ...
      <div class="border border-accent/20 p-2 mb-3">
        ...
        >
          <span class="text-muted">{{ $t('tea_contest.brew_idx', { index: i + 1 }) }}</span>
          <span
            :class="{
          ...
          >
            {{ r.grade === 'perfect' ? $t('tea_contest.grade_perfect') : r.grade === 'good' ? $t('tea_contest.grade_good') : $t('tea_contest.grade_poor') }}
          </span>
          <span class="text-muted">{{ $t('tea_contest.points', { score: r.score }) }}</span>
        </div>
      </div>
      ...
      <Button class="w-full" @click="handleClaim">{{ $t('tea_contest.btn_claim') }}</Button>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const BREW_STEPS = computed(() => [
    {
      label: i18n.global.t('tea_contest.step1_label'),
      shortLabel: i18n.global.t('tea_contest.step1_short'),
      hint: i18n.global.t('tea_contest.step1_hint'),
      action: i18n.global.t('tea_contest.step1_action'),
      lowLabel: i18n.global.t('tea_contest.step1_low'),
      highLabel: i18n.global.t('tea_contest.step1_high')
    },
    {
      label: i18n.global.t('tea_contest.step2_label'),
      shortLabel: i18n.global.t('tea_contest.step2_short'),
      hint: i18n.global.t('tea_contest.step2_hint'),
      action: i18n.global.t('tea_contest.step2_action'),
      lowLabel: i18n.global.t('tea_contest.step2_low'),
      highLabel: i18n.global.t('tea_contest.step2_high')
    },
    {
      label: i18n.global.t('tea_contest.step3_label'),
      shortLabel: i18n.global.t('tea_contest.step3_short'),
      hint: i18n.global.t('tea_contest.step3_hint'),
      action: i18n.global.t('tea_contest.step3_action'),
      lowLabel: i18n.global.t('tea_contest.step3_low'),
      highLabel: i18n.global.t('tea_contest.step3_high')
    }
  ])
  ...
  const currentStepDef = computed(() => BREW_STEPS.value[brewStep.value]!)
