<template>
  <Transition name="panel-fade">
    <div role="presentation" v-if="open" class="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-3" @click.self="$emit('close')">
      <div ref="modalRef_xx7e2" class="map-container game-panel w-full max-w-sm md:max-w-150 max-h-[85vh] overflow-y-auto relative" role="dialog" aria-modal="true" tabindex="-1">
        <button
        ...
        <p class="text-accent text-sm text-center mb-3 tracking-widest">{{ $t('nav.map') }}</p>

        <!-- 田庄 -->
        <div class="map-area">
          <p class="map-area-title">{{ $t('nav.loc_farm') }}</p>
        ...
        <div class="flex space-x-2">
          <div class="map-area flex-1">
            <p class="map-area-title">{{ $t('nav.loc_village') }}</p>
            <div class="map-area-grid">
              <button
          ...
          <div class="map-area flex-1">
            <p class="map-area-title">{{ $t('nav.loc_nature') }}</p>
            <div class="map-area-grid">
              <button
        ...
        <!-- 工坊 -->
        <div class="map-area">
          <p class="map-area-title">{{ $t('nav.loc_workshop') }}</p>
          <div class="map-area-grid">
            <button
        ...
        <!-- 随身 -->
        <div class="map-area">
          <p class="map-area-title">{{ $t('nav.loc_personal') }}</p>
          <div class="map-area-grid">
            <button
...
<script setup lang="ts">
  ...
  const modalRef_xx7e2 = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_xx7e2);
