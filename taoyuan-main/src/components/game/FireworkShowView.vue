<template>
  <div class="game-panel max-w-sm w-full">
    <h3 class="text-accent text-sm mb-3 flex items-center space-x-1">
      <Sparkles :size="14" />
      <span>{{ $t('firework.title') }}</span>
    </h3>

    <!-- 准备 -->
    <div v-if="phase === 'ready'">
      <p class="text-xs text-muted mb-3">{{ $t('firework.desc') }}</p>
      <Button class="w-full" @click="startGame">{{ $t('firework.btn_start') }}</Button>
    </div>

    <!-- 游戏中 -->
    <div v-else-if="phase !== 'finished'">
      <div class="flex items-center justify-between mb-2">
        <p class="text-xs text-muted">{{ $t('firework.round', { current: round + 1, total: 5 }) }}</p>
    ...
    <!-- 最终结果 -->
    <div v-else>
      <p class="text-xs text-muted mb-2">{{ $t('firework.end_title') }}</p>
      ...
      <div class="border border-accent/20 p-3 mb-3 text-center">
        <p class="text-xs mb-1">
          {{ $t('firework.passed_rounds') }}
          <span class="text-success">{{ completedRounds }}</span>
          / 5
        </p>
        <p class="text-xs">
          {{ $t('firework.total_prize') }}
          <span class="text-accent">{{ score }}</span>
          {{ $t('status_bar.money_unit') }}
          <span v-if="completedRounds === 5" class="text-accent finish-flash">{{ $t('firework.all_clear') }}</span>
        </p>
      </div>
      <Button class="w-full" @click="handleClaim">{{ $t('firework.btn_claim') }}</Button>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  /** 记录每bánh xe通过/失败: true=通过, false=失败, null=未到 */
  ...
  const phaseText = computed(() => {
    switch (phase.value) {
      case 'watching':
        return i18n.global.t('firework.watching')
      case 'repeating':
        return i18n.global.t('firework.repeating')
      case 'round_success':
        return i18n.global.t('firework.round_success')
      case 'round_fail':
        return i18n.global.t('firework.round_fail')
      default:
        return ''
    }
  })
  ...
  const startRound = () => {
    const seqLength = round.value + 2 // Không.1bánh xe2một, không.5bánh xe6một
    sequence.value = generateSequence(seqLength)
    playerInput.value = []
    phase.value = 'watching'

    // 展示间隔随bánh xe次加快: Không.1bánh xe500ms → Không.5bánh xe350ms
    ...
    const showNext = () => {
      if (idx < sequence.value.length) {
        ...
        // 回忆时间: 基础5秒 + 每một位置1.5秒，随bánh xe次略减
    ...
    if (idx === expected) {
      ...
      if (playerInput.value.length === sequence.value.length) {
        ...
        phaseTimeout = setTimeout(() => {
          round.value++
          if (round.value >= 5) {
            score.value += 200 // Phần thưởng truy cập tất cả
            sfxRankFirst()
            phase.value = 'finished'
          } else {
            startRound()
