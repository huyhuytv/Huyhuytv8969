<template>
  <div class="game-panel max-w-sm w-full">
    <h3 class="text-accent text-sm mb-3 flex items-center space-x-1">
      <Ship :size="14" />
      <span>{{ $t('dragon_boat.title') }}</span>
    </h3>

    <!-- 准备阶段 -->
    <div v-if="phase === 'ready'">
      <p class="text-xs text-muted mb-3">{{ $t('dragon_boat.desc') }}</p>
      <Button class="w-full" @click="startCountdown">{{ $t('dragon_boat.btn_start') }}</Button>
    </div>

    <!-- 倒计时 -->
    <div v-else-if="phase === 'countdown'" class="text-center py-6">
      <p class="text-xs text-muted mb-3">{{ $t('dragon_boat.ready') }}</p>
    ...
    <div v-else-if="phase === 'racing'">
      <div class="flex items-center justify-between mb-2">
        <p class="text-xs text-muted">
          ...
          {{ $t('dragon_boat.time_left') }}
        </p>
        <p class="text-sm font-bold" :class="timeLeft <= 3 ? 'text-danger time-pulse' : 'text-accent'">{{ timeLeft }}{{ $t('dragon_boat.sec') }}</p>
      </div>
        ...
        <div v-for="(boat, i) in boats" :key="i" class="mb-2 last:mb-0">
          <div class="flex items-center space-x-2 mb-0.5">
            <span class="text-xs w-14" :class="boatColor(i)">
              {{ boat.name === 'you' ? $t('harvest_fair.you') : $t(`dragon_boat.${boat.name}`) }}
            </span>
            <span class="text-xs text-muted flex-1 text-right">{{ boat.progress }}{{ $t('dragon_boat.dist_unit') }}</span>
          </div>
          <!-- 赛道条 -->
      ...
      <div class="flex items-center space-x-2">
        <Button class="flex-1 py-3 text-sm active:!bg-accent active:!text-bg paddle-btn" :icon="Zap" @click="paddle">{{ $t('dragon_boat.btn_paddle') }}</Button>
        <div class="text-center min-w-12">
          <p class="text-accent text-sm font-bold">{{ clickCount }}</p>
          <p class="text-xs text-muted leading-none">{{ $t('dragon_boat.clicks') }}</p>
        </div>
      </div>
    </div>
    ...
    <!-- 结束 -->
    <div v-else>
      <p class="text-xs text-muted mb-2">{{ $t('dragon_boat.end_title') }}</p>
      ...
      <div class="border border-accent/20 mb-3">
        <div
          ...
          :class="{ 'bg-accent/5': entry.name === 'you' }"
        >
          <div class="flex items-center space-x-2">
            ...
            <span :class="{ 'text-accent': entry.name === 'you' }">{{ entry.name === 'you' ? $t('harvest_fair.you') : $t(`dragon_boat.${entry.name}`) }}</span>
          </div>
          <span class="text-muted">{{ entry.progress }}{{ $t('dragon_boat.dist_unit') }}</span>
        </div>
      </div>

      <div class="border border-accent/20 p-2 mb-3 text-center">
        <span v-if="playerRank === 1" class="text-accent text-xs finish-flash">{{ $t('dragon_boat.rank_1') }}{{ $t('status_bar.money_unit') }}</span>
        <span v-else-if="playerRank === 2" class="text-success text-xs">{{ $t('dragon_boat.rank_2') }}{{ $t('status_bar.money_unit') }}</span>
        <span v-else class="text-muted text-xs">{{ $t('dragon_boat.rank_3') }}{{ $t('status_bar.money_unit') }}</span>
      </div>

      <Button class="w-full" @click="handleClaim">{{ $t('dragon_boat.btn_claim') }}</Button>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const boats = ref<Boat[]>([
    { name: 'you', progress: 0 },
    { name: 'team1', progress: 0 },
    { name: 'team2', progress: 0 }
  ])
  ...
  const playerRank = computed(() => {
    const idx = rankings.value.findIndex(b => b.name === 'you')
  ...
  const endRace = () => {
    ...
    // 按排名播放音效
    const rank = rankings.value.findIndex(b => b.name === 'you') + 1
