<template>
  ...
  >
    ...
    <!-- 互动节日 -->
    <Transition name="panel-fade">
      <div role="presentation" v-if="currentFestival" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
    ...
    <!-- 宠物领养弹窗 -->
    <Transition name="panel-fade">
      <div role="presentation" v-if="pendingPetAdoption" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
        <div ref="modalRef_jokji" class="game-panel max-w-xs w-full text-center" role="dialog" aria-modal="true" tabindex="-1">
          <Divider title :label="$t('app.pet_visitor')" />
          <p class="text-xs leading-relaxed mb-3">{{ $t('app.pet_visitor_desc') }}</p>
          <div class="flex space-x-3 justify-center mb-3">
            <Button :class="petChoice === 'cat' ? '!bg-accent !text-bg' : ''" @click="petChoice = 'cat'">{{ $t('app.cat') }}</Button>
            <Button :class="petChoice === 'dog' ? '!bg-accent !text-bg' : ''" @click="petChoice = 'dog'">{{ $t('app.dog') }}</Button>
          </div>
          <div v-if="petChoice" class="mb-3">
            <p class="text-xs text-muted mb-1">{{ $t('app.name_it') }}</p>
            <input
              ...
              :placeholder="petChoice === 'cat' ? ($i18n.locale === 'vi' ? 'Tiểu Hoa' : 'bông hoa') : ($i18n.locale === 'vi' ? 'Vượng Tài' : 'Thịnh vượng')"
              maxlength="8"
            />
          </div>
          <Button :disabled="!petChoice" @click="confirmPetAdoption">{{ $t('app.adopt') }}</Button>
        </div>
      </div>
    </Transition>
    ...
    <!-- 子女提议弹窗 -->
    <Transition name="panel-fade">
      <div role="presentation" v-if="childProposalVisible" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
        <div ref="modalRef_dowqk" class="game-panel max-w-xs w-full text-center" role="dialog" aria-modal="true" tabindex="-1">
          <Divider title :label="$t('app.family_proposal')" />
          <p class="text-xs leading-relaxed mb-4">{{ proposalSpouseName }}{{ $t('app.child_proposal_msg') }}</p>
          <div class="flex flex-col space-y-1.5">
            <Button class="w-full justify-center" @click="handleChildProposalResponse('accept')">{{ $t('app.agree_child') }}</Button>
            <Button class="w-full justify-center" @click="handleChildProposalResponse('wait')">{{ $t('app.wait_child') }}</Button>
            <Button class="w-full justify-center text-muted" @click="handleChildProposalResponse('decline')">{{ $t('app.decline_child') }}</Button>
          </div>
        </div>
      </div>
    </Transition>
    ...
    <!-- 晨间选项事件弹窗 -->
    <Transition name="panel-fade">
      <div role="presentation" v-if="pendingFarmEvent" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
        <div ref="modalRef_ovbts" class="game-panel max-w-xs w-full text-center" role="dialog" aria-modal="true" tabindex="-1">
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_9w8xg" class="game-panel max-w-sm w-full" role="dialog" aria-modal="true" tabindex="-1">
            ...
            >
              <div class="flex items-center justify-between mb-1">
                <div class="flex items-center space-x-1.5">
                  ...
                  <span v-if="vc.voidRole === 'input'" class="text-[10px] px-1 border border-accent/30 rounded-xs text-accent">{{ $t('app.raw_material_chest') }}</span>
              ...
              <template v-if="expandedVoidChestId === vc.id">
                ...
                <div v-else class="flex flex-col items-center justify-center py-4">
                  ...
                  <p class="text-[10px] text-muted">{{ $t('app.chest_empty') }}</p>
                  <p class="text-[10px] text-muted/50 mt-0.5">{{ $t('app.chest_empty_desc') }}</p>
                </div>
                <Button
          ...
          <div v-if="voidChests.length === 0" class="flex flex-col items-center justify-center py-8">
            ...
            <p class="text-xs text-muted">{{ $t('app.no_void_chest') }}</p>
            <p class="text-[10px] text-muted/50 mt-0.5">{{ $t('app.no_void_chest_desc') }}</p>
          </div>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_c5uix" class="game-panel max-w-sm w-full" role="dialog" aria-modal="true" tabindex="-1">
          <div class="flex items-center justify-between mb-2">
            <p class="text-sm text-accent">{{ $t('app.deposit') }}</p>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_irj4k" class="game-panel max-w-xs w-full" role="dialog" aria-modal="true" tabindex="-1">
          <div class="flex items-center justify-between mb-2">
            <p class="text-sm text-accent">{{ voidQtyModal.mode === 'withdraw' ? $t('app.withdraw') : $t('app.deposit') }}</p>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between mb-1.5">
              <span class="text-xs text-muted">{{ $t('app.qty') }}</span>
              <div class="flex items-center space-x-1">
            ...
            <div class="flex space-x-1">
              <Button class="flex-1 justify-center" :disabled="voidQty <= 1" @click="setVoidQty(1)">{{ $t('app.min') }}</Button>
          ...
          <Button class="w-full justify-center !bg-accent !text-bg" @click="confirmVoidQty">
            {{ voidQtyModal.mode === 'withdraw' ? $t('app.withdraw') : $t('app.deposit') }} &times;{{ voidQty }}
          </Button>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_ysdp2" class="game-panel max-w-xs w-full relative" role="dialog" aria-modal="true" tabindex="-1">
          <button aria-label="Close" class="absolute top-2 right-2 text-muted hover:text-text" @click="voidItemDetail = null">
            <X :size="14" />
          </button>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            ...
            <div v-if="voidItemDetail.quality !== 'normal'" class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">{{ $t('app.quality') }}</span>
              <span class="text-xs" :class="voidQualityClass(voidItemDetail.quality)">
                {{ $t(`app.quality_${voidItemDetail.quality}`) }}
              </span>
            </div>
            <div v-if="voidItemDef.sellPrice" class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">{{ $t('app.sell_price') }}</span>
              <span class="text-xs text-accent">{{ voidItemDef.sellPrice }}{{ $i18n.locale === 'vi' ? ' xu' : ' xu' }}</span>
            </div>
            <div v-if="voidItemDef.staminaRestore" class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">{{ $t('app.stamina_restore') }}</span>
              <span class="text-xs text-success">
                +{{ voidItemDef.staminaRestore }}{{ $t('app.stamina') }}
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_xmllo" class="game-panel max-w-md w-full max-h-[80vh] flex flex-col relative" role="dialog" aria-modal="true" tabindex="-1">
          <button aria-label="Close" class="absolute top-2 right-2 text-muted hover:text-text" @click="showLogModal = false">
            <X :size="14" />
          </button>
          ...
          <div class="flex-1 overflow-y-auto min-h-0">
            <div v-if="groupedLogs.length === 0" class="flex flex-col items-center justify-center py-8 text-muted">
              ...
              <p class="text-xs">{{ $t('app.no_log') }}</p>
            </div>
            <div v-for="(group, gi) in groupedLogs" :key="gi" class="mb-3">
              <div class="flex items-center justify-between">
                ...
                <button
                  ...
                  :title="$i18n.locale === 'vi' ? 'Xóa nhật ký ngày này' : 'Xóa nhật ký cho ngày hôm đó'"
    ...
    <!-- 日志清空确认 -->
    <Transition name="panel-fade">
      <div role="presentation" v-if="clearLogTarget !== undefined" class="fixed inset-0 bg-black/70 flex items-center justify-center z-[60] p-4">
        <div ref="modalRef_uit4w" class="game-panel max-w-xs w-full text-center" role="dialog" aria-modal="true" tabindex="-1">
          <p class="text-xs leading-relaxed mb-4">
            {{ clearLogTarget === null ? $t('app.confirm_clear_all') : $t('app.confirm_clear_') + clearLogTarget + '?' }}
          </p>
          <div class="flex space-x-3 justify-center">
            <Button @click="clearLogTarget = undefined">{{ $t('app.cancel') }}</Button>
            <Button class="btn-danger" :icon="Trash2" :icon-size="12" @click="executeClearLogs">{{ $t('app.confirm') }}</Button>
          </div>
        </div>
      </div>
    </Transition>
    ...
    <!-- 休息确认 -->
    <Transition name="panel-fade">
      <div role="presentation" v-if="showSleepConfirm" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
        <div ref="modalRef_j5h16" class="game-panel max-w-xs w-full text-center" role="dialog" aria-modal="true" tabindex="-1">
          ...
          <div class="flex space-x-3 justify-center mt-4">
            <Button :icon="X" :icon-size="12" @click="showSleepConfirm = false">{{ $t('app.sleep_wait') }}</Button>
...
<script setup lang="ts">
  ...
  const modalRef_jokji = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_jokji);
  const modalRef_dowqk = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_dowqk);
  const modalRef_ovbts = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_ovbts);
  const modalRef_9w8xg = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_9w8xg);
  const modalRef_c5uix = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_c5uix);
  const modalRef_irj4k = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_irj4k);
  const modalRef_ysdp2 = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_ysdp2);
  const modalRef_xmllo = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_xmllo);
  const modalRef_uit4w = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_uit4w);
  const modalRef_j5h16 = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_j5h16);
  ...
  /** 日志清空确认：undefined=不显示, null=清空全部, string=清空指定ngày */
  ...
  // 注册ngày数标签获取器
  _registerDayLabelGetter(() => i18n.global.locale.value === 'vi' ? `Năm thứ ${gameStore.year} - ${SEASON_NAMES[gameStore.season]} - Ngày ${gameStore.day}` : `Không.${gameStore.year}năm ${SEASON_NAMES[gameStore.season]} Không.${gameStore.day}ngày`)

  /** 按ngày分组的日志（最新ngày在前，每ngày内也倒序） */
  ...
  const sleepLabel = computed(() => {
    if (gameStore.hour >= 24) return i18n.global.t('app.sleep_passout')
    if (gameStore.hour >= 20) return i18n.global.t('app.sleep_home')
    return i18n.global.t('app.sleep')
  })

  const sleepSummary = computed(() => {
    if (playerStore.stamina <= 0 || gameStore.hour >= 26) {
      return i18n.global.t('app.sleep_passout_desc')
    }
    if (gameStore.hour >= 24) {
      return i18n.global.t('app.sleep_late_desc')
    }
    return i18n.global.t('app.sleep_normal_desc')
  })
  ...
  const sleepWarning = computed(() => {
    ...
    if (playerStore.stamina <= 0 || gameStore.hour >= 26) {
      ...
      if (pct < 100) {
        warnings.push(i18n.global.locale.value === 'vi' ? `Chỉ hồi phục ${pct}% thể lực, mất ${penaltyPct}% tiền (tối đa ${PASSOUT_MONEY_PENALTY_CAP} xu)` : `Thể lực chỉ hồi phục${pct}%, và mất mát${penaltyPct}%Đồng xu (giới hạn trên${PASSOUT_MONEY_PENALTY_CAP}văn bản)`)
      } else {
        warnings.push(i18n.global.locale.value === 'vi' ? `Mất ${penaltyPct}% tiền (tối đa ${PASSOUT_MONEY_PENALTY_CAP} xu)` : `mất mát${penaltyPct}%Đồng xu (giới hạn trên${PASSOUT_MONEY_PENALTY_CAP}văn bản)`)
      }
      ...
      if (pct < 100) {
        warnings.push(i18n.global.locale.value === 'vi' ? `Chỉ hồi phục ${pct}% thể lực` : `Thể lực chỉ hồi phục${pct}%`)
      }
    }
    // Không.28ngày换季警告：统计将枯萎的作物
    if (gameStore.day === 28) {
      ...
      if (willWitherCount > 0) {
        ...
        let msg = i18n.global.locale.value === 'vi' ? `Ngày mai vào mùa ${nextName}! ${willWitherCount} cây trồng sẽ bị héo!` : `Nhập vào ngày mai${nextName}mùa,${willWitherCount}Mùa màng sẽ khô héo!`
        if (harvestableCount > 0) {
          msg += i18n.global.locale.value === 'vi' ? ` (Trong đó ${harvestableCount} cây đã có thể thu hoạch)` : `(Trong số đó${harvestableCount}Cây đã sẵn sàng cho thu hoạch)`
        }
        warnings.push(msg)
      }
    }
  ...
  const confirmPetAdoption = () => {
    ...
    const defaultName = petChoice.value === 'cat' ? (i18n.global.locale.value === 'vi' ? 'Tiểu Hoa' : 'bông hoa') : (i18n.global.locale.value === 'vi' ? 'Vượng Tài' : 'Thịnh vượng')
  ...
  const proposalSpouseName = computed(() => {
    ...
    if (!spouse) return i18n.global.locale.value === 'vi' ? 'Vợ/Chồng' : 'vợ/chồng'
    return getNpcById(spouse.npcId)?.name ?? (i18n.global.locale.value === 'vi' ? 'Vợ/Chồng' : 'vợ/chồng')
  })
  ...
  const handleChildProposalResponse = (response: 'accept' | 'decline' | 'wait') => {
    ...
    if (result.friendshipChange !== 0) {
      addLog(i18n.global.locale.value === 'vi' ? `(Thân thiết ${result.friendshipChange > 0 ? '+' : ''}${result.friendshipChange})` : `(ấn tượng tốt${result.friendshipChange > 0 ? '+' : ''}${result.friendshipChange})`)
    }
    closeChildProposal()
  }
  ...
  const VOID_QUALITY_LABEL: Record<Quality, string> = {
    normal: i18n.global.locale.value === 'vi' ? 'Thường' : 'Thường',
    fine: i18n.global.locale.value === 'vi' ? 'Tốt' : 'Tốt',
    excellent: i18n.global.locale.value === 'vi' ? 'Xuất sắc' : 'Cửa hàng nhỏ',
    supreme: i18n.global.locale.value === 'vi' ? 'Hoàn hảo' : 'Hoàn hảo'
  }
  ...
  const handleVoidDepositDuplicates = () => {
    ...
    if (totalDeposited > 0) {
      addLog(i18n.global.locale.value === 'vi' ? `Đã cất ${kindCount} loại vật phẩm, tổng cộng ${totalDeposited} cái vào Rương hư không.` : `Gửi tiền bằng một cú nhấp chuột${kindCount}mặt hàng, tổng cộng${totalDeposited}Đi đến Hộp trống.`)
    } else {
      addLog(i18n.global.locale.value === 'vi' ? 'Rương hư không đã đầy, không thể cất vào.' : 'Rương trống đã đầy và không thể gửi được.')
    }
  }
  ...
  const executeVoidWithdraw = (chestId: string, itemId: string, quality: Quality, qty: number) => {
    if (!warehouseStore.withdrawFromChest(chestId, itemId, qty, quality)) {
      addLog(i18n.global.t('gameLayout.inventory_full_take') || (i18n.global.locale.value === 'vi' ? 'Balo đã đầy, không thể lấy ra.' : 'Ba lô đã đầy và không thể lấy ra được.'))
      return
    }
    addLog(i18n.global.locale.value === 'vi' ? `Lấy ra ${getItemName(itemId)}×${qty} từ Rương hư không.` : `Lấy từ Rương Hư Không${getItemName(itemId)}×${qty}.`)
  }
  ...
  const executeVoidDeposit = (chestId: string, itemId: string, quality: Quality, qty: number) => {
    ...
    addLog(i18n.global.locale.value === 'vi' ? `Đã cất ${getItemName(itemId)}×${actualQty} vào Rương hư không.` : `Đã gửi tiền${getItemName(itemId)}×${actualQty}Đi đến Rương trống.`)
