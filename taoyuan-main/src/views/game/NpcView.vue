<template>
  <div>
    ...
    <div class="flex space-x-1.5 mb-3">
      ...
      >
        {{ $i18n.locale === 'vi' ? 'Dân làng' : 'dân làng' }}
      </Button>
      <Button
        class="flex-1 justify-center"
    ...
    <!-- NPC 交互弹窗 -->
    <Transition name="panel-fade">
      <div role="presentation" v-if="selectedNpc" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" @click.self="selectedNpc = null">
        <div ref="modalRef_s0uwi" class="game-panel max-w-lg w-full max-h-[80vh] overflow-y-auto" role="dialog" aria-modal="true" tabindex="-1">
          <!-- 头部：名称 + Đóng -->
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <div class="flex items-center space-x-1.5 flex-wrap">
              ...
              >
                ...
                <span>{{ selectedNpcState?.talkedToday ? ($i18n.locale === 'vi' ? 'Đã trò chuyện' : 'Đã trò chuyện') : ($i18n.locale === 'vi' ? 'Có thể trò chuyện' : 'Có thể trò chuyện') }}</span>
              </span>
              ...
              >
                <Cake :size="10" />
                <span>{{ SEASON_NAMES_MAP[selectedNpcDef.birthday.season] }}{{ selectedNpcDef.birthday.day }}{{ $i18n.locale === 'vi' ? ' ngày' : 'Ngày' }}</span>
              </span>
              <span v-if="npcStore.isBirthday(selectedNpc!)" class="text-[10px] text-danger border border-danger/30 rounded-xs px-1">
                {{ $i18n.locale === 'vi' ? 'Sinh nhật! Tặng quà x4' : 'sinh nhật! tặng quà×4' }}
              </span>
            </div>
          </div>
          ...
          <div v-if="selectedNpcState && selectedNpcState.triggeredHeartEvents.length > 0" class="mb-3">
            <p class="text-xs text-muted mb-1">{{ $i18n.locale === 'vi' ? 'Ký ức:' : 'Ký ức:' }}</p>
          ...
          <div class="mb-3 flex space-y-2 flex-wrap">
            <Button class="w-full" :icon="MessageCircle" :disabled="selectedNpcState?.talkedToday" @click="handleTalk">
              {{ selectedNpcState?.talkedToday ? ($i18n.locale === 'vi' ? 'Hôm nay đã trò chuyện' : 'Đã trò chuyện hôm nay') : ($i18n.locale === 'vi' ? 'Trò chuyện' : 'Trò chuyện') }}
            </Button>
            <!-- 每日提示按钮 -->
            <Button
            ...
            >
              {{ selectedNpc && npcStore.isTipGivenToday(selectedNpc) ? ($i18n.locale === 'vi' ? 'Hôm nay đã gợi ý' : 'Hôm nay đã nhắc rồi') : TIP_NPC_LABELS[selectedNpc as TipNpcId] }}
            </Button>
            <!-- 离婚按钮 -->
          ...
          <p v-if="npcStore.weddingCountdown > 0 && npcStore.weddingNpcId === selectedNpc" class="text-xs text-accent mb-3">
            Lễ cưới sẽ diễn ra vào lúc {{ npcStore.weddingCountdown }} Nữ hoàng giữ nó!
          </p>
          ...
          >
            ...
            <template v-if="!selectedNpcState?.dating && !(npcStore.weddingCountdown > 0 && npcStore.weddingNpcId === selectedNpc)">
              <p v-if="npcStore.npcStates.some(s => s.married)" class="text-[10px] text-muted/50 mb-1">Bạn đã có đối tác rồi nên không thể đưa thêm chiếc khăn tay nữa.</p>
              <template v-else>
                <div class="flex flex-col space-y-0.5 mb-1.5">
                  ...
                  >
                    ...
                    <span>ấn tượng tốt≥2000(8trái tim)</span>
                  ...
                  >
                    ...
                    <Circle v-else :size="10" />
                    <span>Cầm chiếc khăn lụa</span>
                    <span class="text-muted/40">— Có sẵn tại cửa hàng satin</span>
                  </span>
                </div>
                <Button class="w-full text-danger border-danger/40" :icon="Heart" :disabled="!canStartDating" @click="handleStartDating">
                  Trao chiếc khăn tay (bắt đầu hẹn hò)
                </Button>
              </template>
            </template>
            <template v-else-if="selectedNpcState?.dating">
              ...
              <div class="flex flex-col space-y-0.5 mb-1.5">
                ...
                >
                  ...
                  <Circle v-else :size="10" />
                  Cầm chiếc nhẫn ngọc lục bảo
                  <span class="text-muted/40">— Có sẵn tại cửa hàng satin</span>
                </span>
              </div>
              <Button class="w-full text-danger border-danger/40" :icon="Heart" :disabled="!canPropose" @click="handlePropose">Đề xuất</Button>
            </template>
          </div>

          <!-- người bạn tâm giao面板(同性可婚NPC,未约会/未结婚） -->
          <div
            v-if="
          ...
          >
            <p class="text-xs text-accent/80 mb-1.5 flex items-center space-x-1">
              <Heart :size="12" />
              <span>người bạn tâm giao</span>
            </p>
            <template v-if="selectedNpcState?.zhiji">
              <p class="text-[10px] text-accent/60 mb-1">{{ selectedNpcDef.gender === 'male' ? 'Lan Yên tâm sự' : 'người bạn tâm giao' }} ♦</p>
              <Button class="w-full text-danger border-danger/40" @click="showZhijiDissolveConfirm = true">Cạnh bị gãy</Button>
            </template>
            <template v-else-if="npcStore.npcStates.some(s => s.zhiji)">
              <p class="text-[10px] text-muted/50">Bạn đã có một người bạn thân và bạn không thể tạo thêm mối liên hệ nào nữa.</p>
            </template>
            <template v-else>
              <div class="flex flex-col space-y-0.5 mb-1.5">
                <span
          ...
          <div v-if="showZhijiDissolveConfirm" class="game-panel mb-3 border-accent/40">
            <p class="text-xs text-danger mb-2">Hãy chắc chắn rằng bạn muốn đi cùng{{ selectedNpcDef?.name }}Mối quan hệ tan vỡ? (chi phí10000văn bản)</p>
          ...
          <div v-if="showDivorceConfirm" class="game-panel mb-3 border-danger/40">
            <p class="text-xs text-danger mb-2">Hãy chắc chắn rằng bạn muốn đi cùng{{ selectedNpcDef?.name }}Hòa giải? (chi phí30000văn bản)</p>
            <div class="flex space-x-2">
          ...
          <div>
            <p class="text-xs text-muted mb-2">
              Tặng quà (chọn đồ trong ba lô)
              <span v-if="npcStore.isBirthday(selectedNpc!)" class="text-danger">— Đang tiến hành phần thưởng sinh nhật!</span>
            </p>
            <template v-if="selectedNpcState?.giftedToday">
              <div class="flex flex-col items-center justify-center py-6 text-muted">
                ...
                <p class="text-xs">Hôm nay tôi đã tặng quà.</p>
              </div>
            </template>
            <template v-else-if="(selectedNpcState?.giftsThisWeek ?? 0) >= 2">
              <div class="flex flex-col items-center justify-center py-6 text-muted">
                <Gift :size="32" class="mb-2" />
                <p class="text-xs">Đã gửi trong tuần này2Đây là một món quà.</p>
              </div>
            </template>
            <template v-else>
              ...
              <div v-if="giftableItems.length === 0" class="flex flex-col items-center justify-center py-6 text-muted">
                ...
                <p class="text-xs">Ba lô trống rỗng</p>
              </div>
            </template>
          </div>
          ...
          <Transition name="panel-fade">
            ...
            >
              <div ref="modalRef_8z7zl" class="game-panel max-w-xs w-full relative" role="dialog" aria-modal="true" tabindex="-1">
                <button aria-label="Close" class="absolute top-2 right-2 text-muted hover:text-text" @click="activeGiftKey = null">
                  <X :size="14" />
                </button>
                ...
                <div v-if="activeGiftReaction" class="border border-accent/10 rounded-xs p-2 mb-2">
                  <div class="flex items-center justify-between">
                    <span class="text-xs text-muted">{{ selectedNpcDef?.name }}cảm thấy</span>
...
<script setup lang="ts">
  ...
  const modalRef_s0uwi = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_s0uwi);
  const modalRef_8z7zl = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_8z7zl);
  ...
  const tutorialHint = computed(() => {
    ...
    if (npcStore.npcStates.every(n => n.friendship === 0)) return 'Bấm vào hình đại diện của dân làng để trò chuyện và gửi quà. Tương tác thường xuyên có thể làm tăng tình bạn.'
    return null
  })
  ...
  const giftTagText = computed(() => {
    const state = selectedNpcState.value
    if ((state?.giftsThisWeek ?? 0) >= 2) return 'Đã được giao trong tuần này'
    if (state?.giftedToday) return 'Đã gửi hôm nay'
    return `Có thể dùng làm quà tặng ${state?.giftsThisWeek ?? 0}/2`
  })
  ...
  const SEASON_NAMES_MAP: Record<string, string> = { spring: 'Xuân', summer: 'Hạ', autumn: 'Thu', winter: 'Đông' }
  ...
  const handleTalk = () => {
    ...
    if (gameStore.isPastBedtime) {
      addLog('Đã quá muộn, mọi người đều đã ngủ.')
      handleEndDay()
      return
    }
    ...
    if (result) {
      ...
      addLog(`với${selectedNpcDef.value?.name}Trò chuyện.(+${result.friendshipGain}ấn tượng tốt)`)
  ...
  const handleDailyTip = () => {
    ...
    if (tip) {
      ...
      addLog(`${selectedNpcDef.value?.name}Đã cho bạn một số thông tin hữu ích.`)
    }
  }
  ...
  const handleGift = (itemId: string, quality: Quality = 'normal') => {
    ...
    if (result) {
      ...
      if (result.gain > 0) {
        addLog(`đưa cho${npcName}${itemName},${npcName}cảm thấy${result.reaction}.(+${result.gain}ấn tượng tốt)`)
      } else if (result.gain < 0) {
        addLog(`đưa cho${npcName}${itemName},${npcName}${result.reaction}cái này……(${result.gain}ấn tượng tốt)`)
      } else {
        addLog(`đưa cho${npcName}${itemName},${npcName}cảm thấy${result.reaction}.`)
      }
