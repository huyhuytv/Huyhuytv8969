<template>
  <div>
    ...
    <div class="space-y-3" role="list">
      <div v-for="skill in skillStore.skills" 
           :key="skill.type" 
           class="game-panel focus:outline-none focus:ring-2 focus:ring-accent/40" 
           role="listitem"
           tabindex="0"
           :aria-label="getSkillAriaLabel(skill)">
...
<script setup lang="ts">
  ...
  const getSkillAriaLabel = (skill: any) => {
    let text = `${t('skills.' + skill.type)} Lv.${skill.level}. `
    
    const info = expInfo(skill.type)
    text += (info ? `${info.current} / ${info.required}. ` : `${t('skillView.max_level')}. `)
    
    text += `${t('skillView.descs.' + skill.type)}. `
    text += `${t('skillView.bonus_prefix')}${t('skillView.bonuses.' + skill.type)}. `
    
    if (skill.perk5 || skill.perk10) {
      if (skill.perk5) text += `Lv5: ${t('skills.' + skill.perk5)}. ${t('skills.' + skill.perk5 + '_desc')}. `
      if (skill.perk10) text += `Lv10: ${t('skills.' + skill.perk10)}. ${t('skills.' + skill.perk10 + '_desc')}. `
    } else if (skill.level < 5) {
      text += `${t('skillView.perk_level5_10')}. `
    } else {
      text += `${t('skillView.upgrade_to_perk', { level: !skill.perk5 ? 5 : 10 })}. `
    }
