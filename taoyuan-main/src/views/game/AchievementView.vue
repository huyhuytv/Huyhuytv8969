  ...
  <div>
    <div class="flex items-center justify-between mb-1">
      <div class="flex items-center space-x-1.5 text-sm text-accent">
        <BookOpen :size="14" />
        <span>{{ $t('achievementView.title') }}</span>
      </div>
    ...
    <div class="flex space-x-1 mb-3">
      <Button class="flex-1 justify-center" :class="{ '!bg-accent !text-bg': tab === 'collection' }" @click="tab = 'collection'">
        {{ $t('achievementView.tab_collection') }}
      </Button>
      <Button class="flex-1 justify-center" :class="{ '!bg-accent !text-bg': tab === 'achievements' }" @click="tab = 'achievements'">
        {{ $t('achievementView.tab_achievements') }}
      </Button>
      <Button class="flex-1 justify-center" :class="{ '!bg-accent !text-bg': tab === 'bundles' }" @click="tab = 'bundles'">{{ $t('achievementView.tab_bundles') }}</Button>
      <Button class="flex-1 justify-center" :class="{ '!bg-accent !text-bg': tab === 'shipping' }" @click="tab = 'shipping'">{{ $t('achievementView.tab_shipping') }}</Button>
      <Button class="flex-1 justify-center" :class="{ '!bg-accent !text-bg': tab === 'notes' }" @click="tab = 'notes'">{{ $t('achievementView.tab_notes') }}</Button>
    </div>
    ...
    <template v-if="tab === 'collection'">
      <div class="flex items-center justify-between mb-1.5">
        <p class="text-xs text-muted">
          {{ $t('achievementView.discovered', { discovered: filteredDiscoveredCount, total: filteredItems.length }) }}
          <span v-if="selectedCategory" class="text-accent ml-0.5">· {{ $t('inventoryView.categories.' + selectedCategory) }}</span>
        </p>
        <button
        ...
        >
          {{ $t('achievementView.clear_filter') }}
        </button>
      </div>
      <div class="flex flex-wrap mb-2">
        ...
        >
          {{ $t('inventoryView.categories.' + cat.key) }}({{ cat.count }})
        </button>
      </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_4q9r6" class="game-panel max-w-xs w-full relative" role="dialog" aria-modal="true" tabindex="-1">
          <button aria-label="Close" class="absolute top-2 right-2 text-muted hover:text-text" @click="activeCollectionId = null">
            <X :size="14" />
          </button>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs">{{ $t('inventoryView.categories.' + activeCollectionItem.category) }}</span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">{{ $t('achievementView.sell_price') }}</span>
              <span class="text-xs text-accent">{{ activeCollectionItem.sellPrice }}{{ $t('status_bar.money_unit') }}</span>
            </div>
            <div v-if="activeCollectionItem.edible && activeCollectionItem.staminaRestore" class="flex items-center justify-between mt-0.5">
              ...
              <span class="text-xs text-success">
                +{{ activeCollectionItem.staminaRestore }}{{ $t('inventoryView.stamina') }}
            ...
            <template v-if="activeWeaponDef">
              ...
              <div v-if="activeWeaponDef.fixedEnchantment" class="flex items-center justify-between mt-0.5">
                <span class="text-xs text-muted">{{ $t('achievementView.fixed_enchant') }}</span>
            ...
            <div v-if="achievementStore.getDiscoveryTime(activeCollectionItem.id)" class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">{{ $t('achievementView.discovered_at') }}</span>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_jgfj8" class="game-panel max-w-xs w-full relative" role="dialog" aria-modal="true" tabindex="-1">
          <button aria-label="Close" class="absolute top-2 right-2 text-muted hover:text-text" @click="activeAchievement = null">
            <X :size="14" />
          </button>
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <div class="flex items-center justify-between mb-1">
              <span class="text-xs text-muted">{{ $t('achievementView.progress') }}</span>
          ...
          <!-- 奖励 -->
          <div class="border border-accent/10 rounded-xs p-2">
            <p class="text-xs text-muted mb-1">{{ $t('achievementView.reward') }}</p>
            <div class="flex flex-wrap space-x-3">
              <span v-if="activeAchievement.reward.money" class="text-xs text-accent">{{ activeAchievement.reward.money }}{{ $t('status_bar.money_unit') }}</span>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_f2zi8" class="game-panel max-w-xs w-full relative" role="dialog" aria-modal="true" tabindex="-1">
          <button aria-label="Close" class="absolute top-2 right-2 text-muted hover:text-text" @click="activeBundle = null">
            <X :size="14" />
          </button>
          ...
          <!-- 需求物品 -->
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            <p class="text-xs text-muted mb-1">{{ $t('achievementView.required_items') }}</p>
          ...
          <div v-if="!achievementStore.isBundleComplete(activeBundle.id)" class="flex flex-col space-y-1">
            ...
            >
              {{ $t('achievementView.submit_item', { item: getItemName(req.itemId) }) }}
            </Button>
          </div>
          ...
          <div v-else class="border border-success/30 rounded-xs p-2">
            <div class="flex items-center space-x-1">
              ...
              <span class="text-xs text-success">{{ $t('achievementView.completed') }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <template v-if="tab === 'shipping'">
      <p class="text-xs text-muted mb-2">{{ $t('achievementView.shipping_records', { shipped: shopStore.shippedItems.length, total: shippableItems.length }) }}</p>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div ref="modalRef_irbc0" class="game-panel max-w-xs w-full relative" role="dialog" aria-modal="true" tabindex="-1">
          <button aria-label="Close" class="absolute top-2 right-2 text-muted hover:text-text" @click="activeShippingId = null">
            <X :size="14" />
          </button>
          ...
          <div class="border border-accent/10 rounded-xs p-2">
            <div class="flex items-center justify-between">
              <span class="text-xs text-muted">{{ i18n.global.locale.value === 'vi' ? 'Phân loại' : 'Phân loại' }}</span>
              <span class="text-xs">{{ CATEGORY_NAMES[activeShippingItem.category] ?? activeShippingItem.category }}</span>
            </div>
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">{{ i18n.global.locale.value === 'vi' ? 'Giá bán' : 'Giá bán' }}</span>
              <span class="text-xs text-accent">{{ activeShippingItem.sellPrice }}{{ i18n.global.locale.value === 'vi' ? ' xu' : ' xu' }}</span>
            </div>
            <div v-if="activeShippingItem.edible && activeShippingItem.staminaRestore" class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">{{ i18n.global.locale.value === 'vi' ? 'Khôi phục' : 'khôi phục' }}</span>
              <span class="text-xs text-success">
                +{{ activeShippingItem.staminaRestore }}{{ i18n.global.locale.value === 'vi' ? ' Thể lực' : 'sức mạnh thể chất' }}
    ...
    <template v-if="tab === 'notes'">
      <div v-if="secretNoteStore.collectedCount === 0" class="flex flex-col items-center justify-center py-10 space-y-3">
        ...
        <p class="text-sm text-muted">{{ i18n.global.locale.value === 'vi' ? 'Chưa thu thập được bản ghi chép bí mật nào' : 'Chưa có ghi chú bí mật nào được thu thập' }}</p>
        <p class="text-xs text-muted/60 text-center max-w-60">{{ i18n.global.locale.value === 'vi' ? 'Có cơ hội nhận được ghi chép bí mật khi đào mỏ, câu cá hoặc thu hái' : 'Có cơ hội nhận được ghi chú bí mật khi khai thác, câu cá và thu thập.' }}</p>
      </div>
      <template v-else>
    ...
    <!-- 秘密笔记详情弹窗 -->
    <Transition name="panel-fade">
      <div role="presentation" v-if="activeNote" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" @click.self="activeNote = null">
        <div ref="modalRef_80zpl" class="game-panel max-w-xs w-full relative" role="dialog" aria-modal="true" tabindex="-1">
          <button aria-label="Close" class="absolute top-2 right-2 text-muted hover:text-text" @click="activeNote = null">
            <X :size="14" />
          </button>
          ...
          <div v-if="activeNote.usable && !secretNoteStore.isUsed(activeNote.id)" class="mt-2">
            <Button class="w-full justify-center !bg-accent !text-bg" @click="handleUseNote(activeNote.id)">{{ $i18n.locale === 'vi' ? 'Sử dụng' : 'Sử dụng ghi chép' }}</Button>
          </div>
          <div v-else-if="activeNote.usable && secretNoteStore.isUsed(activeNote.id)" class="border border-success/30 rounded-xs p-2">
            <div class="flex items-center space-x-1">
              <CircleCheck :size="12" class="text-success" />
              <span class="text-xs text-success">{{ $i18n.locale === 'vi' ? 'Đã sử dụng' : 'Đã sử dụng' }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="mt-3 border border-accent/20 rounded-xs p-2">
      <div class="flex items-center space-x-2 text-xs mb-1.5">
        <span class="text-xs text-muted shrink-0">{{ $i18n.locale === 'vi' ? 'Mức độ hoàn thành' : 'Tiến độ' }}</span>
      ...
      <div class="grid grid-cols-2 gap-x-3 gap-y-0.5" role="list">
        ...
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">{{ $i18n.locale === 'vi' ? 'Câu cá' : 'Câu cá' }}</span>
          <span class="text-xs">{{ achievementStore.stats.totalFishCaught }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">{{ $i18n.locale === 'vi' ? 'Nấu ăn' : 'Nấu ăn' }}</span>
          <span class="text-xs">{{ achievementStore.stats.totalRecipesCooked }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">{{ $i18n.locale === 'vi' ? 'Tổng thu nhập' : 'Tổng thu nhập' }}</span>
          <span class="text-xs">{{ achievementStore.stats.totalMoneyEarned }}{{ $i18n.locale === 'vi' ? ' xu' : ' xu' }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">{{ $i18n.locale === 'vi' ? 'Tầng sâu nhất mỏ' : 'Tầng mỏ sâu nhất' }}</span>
          <span class="text-xs">{{ achievementStore.stats.highestMineFloor }}{{ $i18n.locale === 'vi' ? ' tầng' : 'Tầng' }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">{{ $i18n.locale === 'vi' ? 'Tiêu diệt quái' : 'Tiêu diệt quái' }}</span>
          <span class="text-xs">{{ achievementStore.stats.totalMonstersKilled }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">{{ $i18n.locale === 'vi' ? 'Số lần nhân giống' : 'Số lần nhân giống' }}</span>
          <span class="text-xs">{{ achievementStore.stats.totalBreedingsDone }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">{{ $i18n.locale === 'vi' ? 'Lai tạo mới' : 'Phát hiện lai giống' }}</span>
          <span class="text-xs">{{ achievementStore.stats.totalHybridsDiscovered }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-muted">{{ $i18n.locale === 'vi' ? 'Đời cao nhất' : 'Đời lai cao nhất' }}</span>
          <span class="text-xs">
            {{ achievementStore.stats.highestHybridTier > 0 ? achievementStore.stats.highestHybridTier + ($i18n.locale === 'vi' ? ' đời' : 'Đời') : '-' }}
          </span>
        </div>
      </div>
    </div>
...
<script setup lang="ts">
  ...
  const modalRef_4q9r6 = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_4q9r6);
  const modalRef_jgfj8 = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_jgfj8);
  const modalRef_f2zi8 = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_f2zi8);
  const modalRef_irbc0 = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_irbc0);
  const modalRef_80zpl = ref<HTMLElement | null>(null);
  useFocusTrap(modalRef_80zpl);
  ...
  /** 统计各Phân loại物品数量，生成Phân loại按钮列表 */
  ...
  const NOTE_TYPE_LABELS: Record<string, string> = {
    tip: i18n.global.locale.value === 'vi' ? 'Gợi ý' : 'Mẹo',
    treasure: i18n.global.locale.value === 'vi' ? 'Kho báu' : 'kho báu',
    npc: i18n.global.locale.value === 'vi' ? 'Nhân vật' : 'nhân vật',
    story: i18n.global.locale.value === 'vi' ? 'Cốt truyện' : 'câu chuyện'
  }
  ...
  const EFFECT_NAMES: Record<string, string> = {
    ...
    defense_bonus: i18n.global.locale.value === 'vi' ? 'Giảm ST' : 'Giảm sát thương',
    vampiric: i18n.global.locale.value === 'vi' ? 'Hút máu' : 'Ma cà rồng',
    max_hp_bonus: i18n.global.locale.value === 'vi' ? 'HP Tối đa' : 'tối đaHP',
    stamina_reduction: i18n.global.locale.value === 'vi' ? 'Giảm tiêu hao TT' : 'Giảm gắng sức về thể chất',
    mining_stamina: i18n.global.locale.value === 'vi' ? 'Giảm TT Khai khoáng' : 'Sức chịu đựng khai thác giảm',
    farming_stamina: i18n.global.locale.value === 'vi' ? 'Giảm TT Làm nông' : 'Khả năng chịu đựng của việc farm bị giảm',
    fishing_stamina: i18n.global.locale.value === 'vi' ? 'Giảm TT Câu cá' : 'Sức chịu đựng câu cá giảm',
    crop_quality_bonus: i18n.global.locale.value === 'vi' ? 'Cấp 작 vật' : 'chất lượng cây trồng',
    crop_growth_bonus: i18n.global.locale.value === 'vi' ? 'Tăng trưởng cây trồng' : 'tăng tốc tăng trưởng cây trồng',
    fish_quality_bonus: i18n.global.locale.value === 'vi' ? 'Cấp cá' : 'chất lượng cá',
    fishing_calm: i18n.global.locale.value === 'vi' ? 'Độ hiền hòa' : 'Cá ngoan ngoãn',
    sell_price_bonus: i18n.global.locale.value === 'vi' ? 'Thưởng giá bán' : 'Tăng giá bán',
    shop_discount: i18n.global.locale.value === 'vi' ? 'Chiết khấu shop' : 'cửa hàng giảm giá',
    gift_friendship: i18n.global.locale.value === 'vi' ? 'Tình bạn từ quà' : 'Thiện chí như một món quà',
    monster_drop_bonus: i18n.global.locale.value === 'vi' ? 'Vật phẩm rớt' : 'Giọt quái vật',
    exp_bonus: i18n.global.locale.value === 'vi' ? 'Thưởng kinh nghiệm' : 'tiền thưởng kinh nghiệm',
    treasure_find: i18n.global.locale.value === 'vi' ? 'Phát hiện báu vật' : 'Tỷ lệ phát hiện rương kho báu',
    ore_bonus: i18n.global.locale.value === 'vi' ? 'Quặng bổ sung' : 'Quặng bổ sung',
  ...
  const CATEGORY_NAMES: Record<string, string> = {
    seed: i18n.global.locale.value === 'vi' ? 'Hạt giống' : 'hạt giống',
    crop: i18n.global.locale.value === 'vi' ? 'Nông sản' : 'cây trồng',
    hybrid: i18n.global.locale.value === 'vi' ? 'Cây lai' : 'cây lai',
    fish: i18n.global.locale.value === 'vi' ? 'Cá' : 'cá',
    animal_product: i18n.global.locale.value === 'vi' ? 'Sản phẩm động vật' : 'sản phẩm chăn nuôi',
    ...
    machine: i18n.global.locale.value === 'vi' ? 'Máy móc' : 'máy móc',
    sprinkler: i18n.global.locale.value === 'vi' ? 'Vòi phun' : 'vòi phun nước',
    fertilizer: i18n.global.locale.value === 'vi' ? 'Phân bón' : 'phân bón',
    sapling: i18n.global.locale.value === 'vi' ? 'Cây con' : 'cây non',
    bait: i18n.global.locale.value === 'vi' ? 'Mồi câu' : 'Mồi câu',
    tackle: i18n.global.locale.value === 'vi' ? 'Phao' : 'Phao câu',
    bomb: i18n.global.locale.value === 'vi' ? 'Bom' : 'quả bom',
    fossil: i18n.global.locale.value === 'vi' ? 'Hóa thạch' : 'hóa thạch',
    artifact: i18n.global.locale.value === 'vi' ? 'Đồ cổ' : 'Cổ vật',
    weapon: i18n.global.locale.value === 'vi' ? 'Vũ khí' : 'vũ khí',
    ring: i18n.global.locale.value === 'vi' ? 'Nhẫn' : 'Nhẫn',
    hat: i18n.global.locale.value === 'vi' ? 'Mũ' : 'Mũ',
    shoe: i18n.global.locale.value === 'vi' ? 'Giày' : 'Giày'
  }

  /** 可出货的类别（排除hạt giống、máy móc、工具类） */
  ...
  const getProgressText = (a: (typeof ACHIEVEMENTS)[number]): string => {
    ...
    switch (c.type) {
      ...
      case 'married':
        return npcStore.getSpouse() ? i18n.global.t('achievementView.completed') : i18n.global.t('achievementView.incomplete')
      case 'hasChild':
        return npcStore.children.length > 0 ? i18n.global.t('achievementView.completed') : i18n.global.t('achievementView.incomplete')
      case 'allBundlesComplete':
  ...
  const handleSubmit = (bundleId: string, itemId: string) => {
    ...
    if (achievementStore.submitToBundle(bundleId, itemId, toSubmit)) {
      sfxClick()
      addLog(i18n.global.t('achievementView.log_submit', { bundle: bundle?.name, item: getItemName(itemId), qty: toSubmit }))
      if (achievementStore.isBundleComplete(bundleId)) {
        addLog(i18n.global.t('achievementView.log_bundle_complete', { bundle: bundle?.name }))
      }
    } else {
      addLog(i18n.global.t('achievementView.log_submit_failed'))
    }
  }
</script>
