...
const tick = () => {
  ...
  if (newHour >= PASSOUT_HOUR) {
    ...
    isPaused.value = true
    addLog(tText('Đã sáng sớm rồi2Bấm vào không giữ được và ngã xuống……', 'Đã 2 giờ sáng, bạn không thể trụ được nữa và gục xuống...'))
    handleEndDay()
  ...
  if (!gameStore.midnightWarned && prevHour < MIDNIGHT_HOUR && newHour >= MIDNIGHT_HOUR) {
    ...
    addLog(tText('Đã quá nửa đêm và bạn bắt đầu cảm thấy buồn ngủ……', 'Đã quá nửa đêm và bạn bắt đầu cảm thấy buồn ngủ...'))
  }
}
