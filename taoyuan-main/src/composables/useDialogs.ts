...
export const handlePerkSelect = (perk: SkillPerk5 | SkillPerk10) => {
  ...
  addLog(tText('Đã học một chuyên ngành mới!', 'Đã học một chuyên ngành mới!'))
...
export const closeHeartEvent = (changes: { npcId: string; amount: number }[]) => {
  for (const change of changes) {
    ...
    if (change.amount > 0) {
      addLog(tText(`Sự thuận lợi+${change.amount}`, `Sự thuận lợi+${change.amount}`))
...
export const closeFestival = (prize: number) => {
  if (prize > 0) {
    ...
    showFloat(`+${prize}` + (i18n.global.locale.value==='vi'?' văn':'văn bản'), 'accent')
    addLog(tText(`Tiền thưởng ngày lễ:${prize}Chữ!`, `Tiền thưởng kỳ nghỉ: ${prize}wen! `))
  }
