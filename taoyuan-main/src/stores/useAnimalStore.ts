...
export const useAnimalStore = defineStore('animal', () => {
  ...
  /** 喂食所有động vật（消耗指定饲料，马也需要喂食；从背包+仓库箱子取饲料） */
  ...
  /** 标记所有động vật为已喂食（不消耗饲料，用于晨间雇工/配偶预喂食） */
  const markAllFed = () => {
  ...
  /** 开始chuồng gà孵化 (需chuồng gà ≥ 2 级, 仅限 coop 类蛋) */
  const startIncubation = (itemId: string): { success: boolean; message: string } => {
    ...
    if (!coopBuilding?.built || coopBuilding.level < 2) {
      return { success: false, message: 'Yêu cầu chuồng gà lớn (2cấp) để sử dụng lồng ấp.' }
    }
    if (incubating.value) {
      return { success: false, message: 'Đã có trứng nở trong lồng ấp.' }
    }
    const mapping = INCUBATION_MAP[itemId]
    if (!mapping || mapping.building !== 'coop') {
      return { success: false, message: 'Vật phẩm này không thể nở trong chuồng gà.' }
    }

    const inventoryStore = useInventoryStore()
    if (!inventoryStore.removeItem(itemId, 1)) {
      return { success: false, message: 'Vật phẩm này không có trong ba lô.' }
    }
    ...
    return { success: true, message: `Bắt đầu nở${animalDef?.name ?? 'động vật'}, dự kiến${days}Nữ hoàng nở ra.` }
  }
  ...
  const startBarnIncubation = (itemId: string): { success: boolean; message: string } => {
    ...
    if (!barnBuilding?.built || barnBuilding.level < 2) {
      return { success: false, message: 'Yêu cầu chuồng trại lớn (2cấp) để sử dụng lồng ấp.' }
    }
    if (barnIncubating.value) {
      return { success: false, message: 'Trứng đã nở trong lồng ấp.' }
    }
    const mapping = INCUBATION_MAP[itemId]
    if (!mapping || mapping.building !== 'barn') {
      return { success: false, message: 'Vật phẩm này không thể nở trong chuồng.' }
    }
    ...
    const animalDef = ANIMAL_DEFS.find(d => d.type === mapping.animalType)
    return { success: true, message: `Bắt đầu ấp trong chuồng${animalDef?.name ?? 'động vật'}, dự kiến${days}Nữ hoàng nở ra.` }
  }
  ...
  const grazeAnimals = (): { success: boolean; count: number; message: string; bonusProducts?: { itemId: string; quality: Quality }[] } => {
    if (grazedToday.value) {
      return { success: false, count: 0, message: 'Hôm nay nó đã được chăn thả.' }
    }

    const gameStore = useGameStore()
    if (gameStore.isRainy) {
      return { success: false, count: 0, message: 'Không chăn thả vào những ngày mưa.' }
    }

    // 只有喂食过的động vật可放牧（排除马）
    let grazeable: Animal[]
    if (gameStore.season === 'winter') {
      // 冬季仅牦牛可放牧
      grazeable = animals.value.filter(a => a.wasFed && a.type === 'yak')
      if (grazeable.length === 0) {
        return { success: false, count: 0, message: 'Chỉ có bò yak mới có thể chăn thả vào mùa đông và chúng cần được cho ăn trước.' }
      }
    } else {
      grazeable = animals.value.filter(a => a.wasFed && a.type !== 'horse')
      if (grazeable.length === 0) {
        return { success: false, count: 0, message: 'Không có động vật nào được cho ăn để chăn thả.' }
      }
    }
    ...
    let message = `${grazeable.length}Một con vật vui vẻ ăn cỏ.`
    if (pigCount > 0) {
      message += `lợn tìm thấy${pigCount}Một nấm cục!`
    }
  ...
  const dailyUpdate = (): { products: { itemId: string; quality: Quality }[]; died: string[]; gotSick: string[]; healed: string[] } => {
    ...
    for (const animal of animals.value) {
      ...
      if (animal.hunger >= HUNGER_DEATH_DAYS || animal.sickDays >= SICK_DEATH_DAYS) {
        ...
        continue // Bỏ qua quá trình xử lý tiếp theo và xóa chúng sau
      }

      // 友好度变化
      const friendshipMultiplier = gameStore.farmMapType === 'meadowlands' ? 1.5 : 1.0
      // 仙缘能力：灵抚（gui_nv_3）động vật好感获取+25%
      ...
      if (def && def.produceDays > 0 && animal.wasFed && !animal.sick) {
        ...
        if (animal.daysSinceProduct >= effectiveDays) {
          ...
          // 仙缘结缘：灵织（animal_blessing）động vật产品品质+1
  ...
  const installAutoPetter = (buildingType: AnimalBuildingType): { success: boolean; message: string } => {
    if (buildingType === 'stable') return { success: false, message: 'Chuồng ngựa không thể được trang bị máy vuốt ve tự động.' }
    const building = buildings.value.find(b => b.type === buildingType)
    if (!building || !building.built) return { success: false, message: 'Trước tiên phải xây dựng chuồng trại.' }
    if (building.level < 2) return { success: false, message: 'Yêu cầu chuồng trại lớn (2cấp) có thể được cài đặt.' }
    if (autoPetterBuildings.value.includes(buildingType)) return { success: false, message: 'Chuồng trại đã được trang bị máy vuốt ve tự động.' }
    autoPetterBuildings.value.push(buildingType)
    return { success: true, message: `Máy vuốt ve tự động đã được lắp đặt trên${buildingType === 'coop' ? 'chuồng gà' : 'đồng cỏ'}.` }
  }
