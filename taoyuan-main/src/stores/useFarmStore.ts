...
export const useFarmStore = defineStore('farm', () => {
  ...
  /** 收获，返回cây trồngID（支持多茬cây trồng） */
  const harvestPlot = (plotId: number): { cropId: string | null; genetics: SeedGenetics | null } => {
    ...
    // 如果属于巨型cây trồng组，清除同组所有地块的 giantCropGroup（防止残留）
    ...
    if (crop && crop.regrowth && crop.regrowthDays) {
      ...
      if (crop.maxHarvests && plot.harvestCount >= crop.maxHarvests) {
        ...
        plot.unwateredDays = 0
        plot.giantCropGroup = null
        // seedGenetics 保留（多茬cây trồng继续使用同一基因）
      }
    } else {
      plot.state = 'tilled'
      plot.cropId = null
  ...
  /** 铲除cây trồng：将有cây trồng的地块恢复为已耕状态（保留肥料） */
  const removeCrop = (plotId: number): { cropId: string | null } => {
    ...
    const cropId = plot.cropId
    // 如果属于巨型cây trồng组，清除同组所有地块的 giantCropGroup
    if (plot.giantCropGroup !== null) {
      const groupId = plot.giantCropGroup
      for (const p of plots.value) {
        if (p.giantCropGroup === groupId) p.giantCropGroup = null
  ...
  const applyFertileSoil = (farmingLevel: number): { count: number; fertilizerName: string } => {
    ...
    const fertilizerName = farmingLevel >= 8 ? tText('hormone tăng trưởng tiên tiến', 'hormone tăng trưởng tiên tiến') : farmingLevel >= 5 ? tText('Phân bón chất lượng cao', 'Phân bón chất lượng cao') : tText('phân bón cơ bản', 'phân bón cơ bản')
    let count = 0
  ...
  const dailyUpdate = (isRainy: boolean): { newInfestations: number; pestDeaths: number; newWeeds: number; weedDeaths: number } => {
    ...
    // 仙缘能力：春息（tao_yao_2）春季cây trồng生长加速
    ...
    for (const plot of plots.value) {
      ...
      // 虫害感染检查（未感染的 planted/growing 地块，跳过巨型cây trồng）
      ...
      // 杂草滋生检查（未长草的 planted/growing 地块，跳过巨型cây trồng）
  ...
  const lightningStrike = (): { hit: boolean; absorbed: boolean; cropName?: string } => {
    ...
    const cropName = crop?.name ?? tText('cây trồng', 'mùa vụ')
  ...
  /** 检测并形成巨型cây trồng：3×3 同种 harvestable 且 giantCropEligible，1% 概率 */
  ...
  /** 收获巨型cây trồng：清除同组 9 块，返回cây trồngID和总产出数量 */
  ...
  const dailyFruitTreeUpdate = (currentSeason: Season): { fruits: { fruitId: string; quality: Quality }[] } => {
    ...
    const extraFruit = hiddenNpcStore2.isAbilityActive('tao_yao_1') // Hoa: cây ăn quả+1Năng suất
    const spiritPeachActive = hiddenNpcStore2.isAbilityActive('tao_yao_3') // Spirit Peach: Cây đào có xác suất sinh ra Spirit Peach
