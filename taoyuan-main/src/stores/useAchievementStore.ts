...
export const useAchievementStore = defineStore('achievement', () => {
  ...
  /** 物品发现时间记录 { itemId: "Không.Xnăm mùa xuân Không.Yngày" } */
  ...
  const discoverItem = (itemId: string) => {
    if (!discoveredItems.value.includes(itemId)) {
      ...
      const SEASON_NAMES: Record<string, string> = { spring: tText('mùa xuân', 'mùa xuân'), summer: tText('mùa hè', 'mùa hè'), autumn: tText('mùa thu', 'mùa thu'), winter: tText('mùa đông', 'mùa đông') }
      discoveryTimes.value[itemId] = `Không.${gameStore.year}năm ${SEASON_NAMES[gameStore.season] ?? gameStore.season} Không.${gameStore.day}ngày`
    }
  }
