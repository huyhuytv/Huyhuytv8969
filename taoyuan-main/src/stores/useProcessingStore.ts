...
export const useProcessingStore = defineStore('processing', () => {
  ...
  const startProcessing = (slotIndex: number, recipeId: string, specifiedQuality?: Quality): boolean => {
    ...
    if (!slot || slot.recipeId !== null) return false // Đang xử lý
  ...
  const collectProduct = (slotIndex: number): string | null => {
    ...
    if (slot.machineType === 'seed_maker' && slot.inputItemId) {
      ...
      if (breedingStore.trySeedMakerGeneticSeed(slot.inputItemId, farmingLevel)) {
        addLog(tText('Máy làm hạt giống đã tạo ra thêm một hạt giống nhân giống!', 'Máy làm hạt giống đã tạo ra thêm một hạt giống nhân giống!'))
      }
    }
  ...
  const dailyUpdate = () => {
    ...
    if (collected.length > 0) {
      ...
      const summary = Array.from(counts.entries())
        ...
        .join(',')
      addLog(tText(`Xưởng tự động tính phí:${summary}.`, `Hội thảo tự động tính phí: ${summary}. `))
    }
    if (readyNames.length > 0) {
      ...
      const summary = Array.from(counts.entries())
        .map(([name, count]) => (count > 1 ? `${name}x${count}` : name))
        .join(',')
      addLog(tText(`Xử lý hoàn tất:${summary}, hãy đến xưởng để thu thập nó.`, `Xử lý hoàn tất: ${summary}, hãy đến xưởng để lấy. `))
    }
  }
  ...
  const upgradeWorkshop = (): { success: boolean; message: string } => {
    ...
    if (!upgrade) return { success: false, message: tText('Hội thảo đã đạt đến mức cao nhất.', 'Hội thảo đã đạt đến mức cao nhất.') }
    if (!consumeCraftMaterials(upgrade.materials, upgrade.cost)) return { success: false, message: tText('Không đủ nguyên liệu hoặc tiền đồng.', 'Không đủ nguyên liệu hoặc tiền đồng.') }
    workshopLevel.value = next
    return { success: true, message: tText(`Việc mở rộng xưởng đã hoàn thành! Giới hạn trên của máy được tăng lên${maxMachines.value}Đài Loan.`, `Việc mở rộng xưởng đã hoàn thành! Giới hạn trên của máy được tăng lên ${maxMachines.value}. `) }
  }
