...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  /** 卖出vũ khí（不能卖装备中的vũ khí，不能卖唯一vũ khí） */
  const sellWeapon = (index: number): { success: boolean; message: string } => {
    if (ownedWeapons.value.length <= 1) return { success: false, message: 'Giữ ít nhất một vũ khí.' }
    if (index === equippedWeaponIndex.value) return { success: false, message: 'Vũ khí trong trang bị không thể bán được, vui lòng chuyển đổi trước.' }
    ...
    return { success: true, message: `đã bán${def?.name ?? 'vũ khí'}, nhận được${price}Chữ.` }
  }
  ...
  const addItem = (itemId: string, quantity: number = 1, quality: Quality = 'normal'): boolean => {
    ...
    if (remaining > 0) {
      ...
      showFloat(`Ba lô đã đầy!${name}×${remaining}bị mất`, 'danger')
    } else {
      // 背包快满预警：剩余lưới数 ≤ 3 时提示一次
      const freeSlots = capacity.value - items.value.length
      if (freeSlots <= 3) {
        showFloat(`Ba lô đã gần đầy! Còn lại${freeSlots}lưới`, 'accent')
      }
    }
  ...
  /** 装备chiếc nhẫn到指定槽位（0 或 1），禁止两个槽位装备同defIdchiếc nhẫn */
  const equipRing = (ringIndex: number, slot: 0 | 1): boolean => {
    ...
    // 同一枚chiếc nhẫn在另一个槽位 → 交换
    if (otherSlot.value === ringIndex) {
      otherSlot.value = targetSlot.value // có thể -1
      targetSlot.value = ringIndex
      return true
    }
    // 禁止两个槽位装备同defIdchiếc nhẫn
  ...
  /** 卖出chiếc nhẫn（自动卸下已装备的chiếc nhẫn） */
  const sellRing = (index: number): { success: boolean; message: string } => {
    if (index < 0 || index >= ownedRings.value.length) return { success: false, message: 'Chỉ mục không hợp lệ.' }
    ...
    return { success: true, message: `đã bán${def?.name ?? 'chiếc nhẫn'}, nhận được${price}Chữ.` }
  }

  /** 查询某种装备效果的合计值（chiếc nhẫn+mũ+giày叠加） */
  ...
  /** 查询某种chiếc nhẫn效果的合计值（代理到 getEquipmentBonus，包含mũ/giày加成） */
  ...
  const _getSetPieceCount = (set: (typeof EQUIPMENT_SETS)[number]): number => {
    ...
    // chiếc nhẫn：两个槽位只算一次（避免两个同IDchiếc nhẫn重复计数）
  ...
  const craftRing = (defId: string): { success: boolean; message: string } => {
    ...
    if (!def || !def.recipe) return { success: false, message: 'Chiếc nhẫn này không thể được chế tạo.' }

    // 检查材料
    for (const mat of def.recipe) {
      if (getItemCount(mat.itemId) < mat.quantity) {
        const matName = getItemById(mat.itemId)?.name ?? mat.itemId
        return { success: false, message: `Nguyên liệu không đủ:${matName}.` }
      }
    }
    ...
    if (playerStore.money < def.recipeMoney) {
      return { success: false, message: `Không đủ tiền đồng (cần${def.recipeMoney}chữ).` }
    }
    ...
    // 添加chiếc nhẫn
    addRing(defId)
    return { success: true, message: `tổng hợp${def.name}!` }
  }
  ...
  const sellHat = (index: number): { success: boolean; message: string } => {
    if (index < 0 || index >= ownedHats.value.length) return { success: false, message: 'Chỉ mục không hợp lệ.' }
    ...
    return { success: true, message: `đã bán${def?.name ?? 'mũ'}, nhận được${price}Chữ.` }
  }

  /** 合成mũ */
  const craftHat = (defId: string): { success: boolean; message: string } => {
    const def = getHatById(defId)
    if (!def || !def.recipe) return { success: false, message: 'Chiếc mũ này không thể được chế tạo.' }
    for (const mat of def.recipe) {
      if (getItemCount(mat.itemId) < mat.quantity) {
        const matName = getItemById(mat.itemId)?.name ?? mat.itemId
        return { success: false, message: `Nguyên liệu không đủ:${matName}.` }
  ...
  const sellShoe = (index: number): { success: boolean; message: string } => {
    if (index < 0 || index >= ownedShoes.value.length) return { success: false, message: 'Chỉ mục không hợp lệ.' }
    ...
    return { success: true, message: `đã bán${def?.name ?? 'giày'}, nhận được${price}Chữ.` }
  }

  /** 合成giày */
  const craftShoe = (defId: string): { success: boolean; message: string } => {
    const def = getShoeById(defId)
    if (!def || !def.recipe) return { success: false, message: 'Chiếc giày này không thể được chế tạo.' }
    for (const mat of def.recipe) {
      if (getItemCount(mat.itemId) < mat.quantity) {
        const matName = getItemById(mat.itemId)?.name ?? mat.itemId
        return { success: false, message: `Nguyên liệu không đủ:${matName}.` }
  ...
  const applyEquipmentPreset = (id: string): { success: boolean; message: string } => {
    const preset = equipmentPresets.value.find(p => p.id === id)
    if (!preset) return { success: false, message: 'Giải pháp không tồn tại.' }
    ...
    if (preset.ringSlot1DefId) {
      ...
      else missing.push('chiếc nhẫn1')
    } else {
      unequipRing(0)
    }

    // chiếc nhẫn槽2（禁止与槽1装备同defIdchiếc nhẫn）
    if (preset.ringSlot2DefId) {
      if (preset.ringSlot2DefId === preset.ringSlot1DefId) {
        // 旧方案中两个槽保存了同defIdchiếc nhẫn，现已禁止，跳过槽2
        unequipRing(1)
        missing.push('chiếc nhẫn2(Không tương thích với khe cắm1giống nhau)')
      } else {
        const idx = ownedRings.value.findIndex(r => r.defId === preset.ringSlot2DefId)
        if (idx >= 0) equipRing(idx, 1)
        else missing.push('chiếc nhẫn2')
      }
    } else {
      unequipRing(1)
    }
    ...
    if (missing.length > 0) {
      return { success: true, message: `Đề án được áp dụng「${preset.name}」, nhưng${missing.join(',')}Không còn trong ba lô nữa.` }
    }
    return { success: true, message: `Đề án được áp dụng「${preset.name}」.` }
  }
