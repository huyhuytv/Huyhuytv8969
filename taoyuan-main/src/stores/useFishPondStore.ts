  ...
  const dailyUpdate = (): PondDailyResult => {
    ...
    if (pond.value.breeding) {
      ...
      if (pond.value.breeding.daysLeft <= 0) {
        ...
        if (!parentA || !parentB) {
          result.breedingFailed = tText('Cá bố mẹ chết và sinh sản không thành công.', 'Cá bố mẹ chết và sinh sản không thành công.')
        } else if (fishCount.value >= capacity.value) {
          result.breedingFailed = tText('Ao cá đầy, chăn nuôi thất bại', 'Ao cá đầy, chăn nuôi thất bại')
        } else {
