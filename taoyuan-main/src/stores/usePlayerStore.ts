...
export const usePlayerStore = defineStore('player', () => {
  const playerName = ref(tText('Chưa đặt tên', 'Vô danh'))
  ...
  /** NPC 用来称呼玩家的称谓 */
  const honorific = computed(() => (gender.value === 'male' ? tText('Anh trai', 'Anh trai') : tText('cô gái', 'con gái')))
  ...
  const deserialize = (data: ReturnType<typeof serialize>) => {
    ...
    playerName.value = (data as any).playerName ?? tText('Chưa đặt tên', 'Vô danh')
