...
export const useWalletStore = defineStore('wallet', () => {
  ...
  const checkAndUnlock = (): string[] => {
    ...
    if (!has('merchant_seal') && achievementStore.stats.totalMoneyEarned >= 10000) {
      ...
      newlyUnlocked.push(tText('con dấu thương gia', 'con dấu thương gia'))
    }

    // 草药图鉴：采集等级8
    if (!has('herb_guide') && skillStore.getSkill('foraging').level >= 8) {
      unlock('herb_guide')
      newlyUnlocked.push(tText('Dược liệu của Thần Nông', 'Dược liệu của Thần Nông'))
    }

    // Bùa thợ mỏ：矿洞50层
    if (!has('miners_charm') && miningStore.safePointFloor >= 50) {
      unlock('miners_charm')
      newlyUnlocked.push(tText('Bùa thợ mỏ', 'Bùa thợ mỏ'))
    }
    ...
    if (!has('anglers_token')) {
      ...
      if (fishCount >= 30) {
        ...
        newlyUnlocked.push(tText('Mã thông báo Diao Weng', 'Mã thông báo Diao Weng'))
      }
    }

    // mũ đầu bếp：烹饪10道不同食谱
    if (!has('chefs_hat') && achievementStore.stats.totalRecipesCooked >= 10) {
      unlock('chefs_hat')
      newlyUnlocked.push(tText('mũ đầu bếp', 'mũ đầu bếp'))
    }

    // 大地图腾：收获100次作物
    if (!has('earth_totem') && achievementStore.stats.totalCropsHarvested >= 100) {
      unlock('earth_totem')
      newlyUnlocked.push(tText('vật tổ đất', 'vật tổ đất'))
    }
