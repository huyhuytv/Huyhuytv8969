...
export const SEASON_NAMES: Record<Season, string> = {
  get spring() { return i18n.global.t('time.spring') },
  get summer() { return i18n.global.t('time.summer') },
  get autumn() { return i18n.global.t('time.autumn') },
  get winter() { return i18n.global.t('time.winter') }
}

/** 天气中文名 */
export const WEATHER_NAMES: Record<Weather, string> = {
  get sunny() { return i18n.global.t('time.sunny') },
  get rainy() { return i18n.global.t('time.rainy') },
  get stormy() { return i18n.global.t('time.stormy') },
  get snowy() { return i18n.global.t('time.snowy') },
  get windy() { return i18n.global.t('time.windy') },
  get green_rain() { return i18n.global.t('time.green_rain') }
}
...
export const useGameStore = defineStore('game', () => {
  ...
  const advanceTime = (hours: number): { ok: boolean; passedOut: boolean; message: string } => {
    ...
    if (newHour >= PASSOUT_HOUR) {
      ...
      return { ok: true, passedOut: true, message: tText('Đã sáng sớm rồi2Bấm vào không giữ được và ngã xuống……', 'Đã 2 giờ sáng, bạn không thể trụ được nữa và gục xuống...') }
    }
  ...
  const travelTo = (targetTab: string): { ok: boolean; timeCost: number; passedOut: boolean; message: string } => {
    ...
    const travelMsg = cost > 0 ? tText(`đi đến${targetName}, chi tiêu trên đường${Math.round(cost * 60)}phút, tiêu thụ${staminaCost}Một số sức mạnh thể chất.`, `Để đến ${targetName}, phải mất ${Math.round(cost * 60)} phút và tiêu tốn ${staminaCost} điểm năng lượng. `) : ''
    return {
      ok: true,
      timeCost: cost,
