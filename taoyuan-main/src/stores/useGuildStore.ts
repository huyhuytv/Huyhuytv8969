...
export const useGuildStore = defineStore('guild', () => {
  ...
  const claimGoal = (monsterId: string): boolean => {
    ...
    addLog(tText(`Nhận phần thưởng chiến dịch và nhận thêm ${bonusPoints} Điểm đóng góp.`, `Nhận phần thưởng chiến dịch và nhận thêm điểm đóng góp ${bonusPoints}. `))
    return true
  }
  ...
  const checkLevelUp = () => {
    while (guildLevel.value < GUILD_LEVELS.length) {
      ...
      guildLevel.value++
      addLog(tText(`Cấp độ của Hội thám hiểm đã được nâng lên ${guildLevel.value} Mức độ!`, `Cấp bang hội nhà thám hiểm đã được nâng lên ${guildLevel.value}! `))
    }
  }
  ...
  const buyShopItem = (itemId: string): boolean => {
    ...
    addLog(tText(`Mua ở cửa hàng hội「${item.name}」.`, `Đã mua "${item.name}" trong cửa hàng bang hội. `))
    return true
  }
