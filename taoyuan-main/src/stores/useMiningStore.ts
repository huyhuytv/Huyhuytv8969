...
export const useMiningStore = defineStore('mining', () => {
  ...
  /** 获取当前活跃楼lớp数据（兼容主Của tôivớiMỏ sọ) */
  ...
  const engageRevealedMonster = (index: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    if (inCombat.value) return { success: false, message: 'Không thể khám phá trong khi chiến đấu.', startsCombat: false }

    const tile = floorGrid.value[index]
    if (!tile || tile.state !== 'revealed') return { success: false, message: 'Không thể tham gia chiến đấu.', startsCombat: false }
    if (tile.type !== 'monster' && tile.type !== 'boss') return { success: false, message: 'Không có quái vật trong lưới này.', startsCombat: false }

    const monster = tile.data?.monster
    if (!monster) return { success: false, message: 'Không có quái vật trong lưới này.', startsCombat: false }
    ...
    if (tile.type === 'boss') {
      ...
      combatLog.value = [`BOSSChiến đấu! thử thách lần nữa${monster.name}!(HP: ${monster.hp})${isFirstKill ? '' : '(phiên bản yếu đi)'}`]
      combatIsBoss.value = true
    } else {
      combatLog.value = [`gặp lại${monster.name}!(HP: ${monster.hp})`]
      combatIsBoss.value = false
    }
    inCombat.value = true

    return { success: true, message: `với${monster.name}Chiến đấu!`, startsCombat: true }
  }
  ...
  const revealTile = (index: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    const tile = floorGrid.value[index]
    if (!tile || tile.state !== 'hidden') return { success: false, message: 'Lưới này không thể mở được.', startsCombat: false }
    if (!canRevealTile(index)) return { success: false, message: 'Chỉ có thể mở các vị trí liền kề của lưới đã khám phá.', startsCombat: false }

    // 检查镐是否可用（未在升级中)
    if (!inventoryStore.isToolAvailable('pickaxe')) {
      return { success: false, message: 'Cái cuốc đang được nâng cấp và mỏ không thể khám phá được.', startsCombat: false }
    }

    // 扣sức mạnh thể chất（1 点基础,受镐/技能/buff 减免)
    ...
    // 仙缘能力：聚气（shan_weng_1)挖矿sức mạnh thể chất-15%
    ...
    if (!playerStore.consumeStamina(staminaCost)) {
      return { success: false, message: 'Không đủ sức chịu đựng để khám phá.', startsCombat: false }
    }
    ...
    switch (tile.type) {
      ...
      default:
        tile.state = 'revealed'
        return { success: true, message: 'Không có gì.', startsCombat: false }
    }
  }
  ...
  const _handleEmptyTile = (tile: MineTile, staminaCost: number): { success: boolean; message: string; startsCombat: boolean } => {
    tile.state = 'revealed'
    return { success: true, message: `Một khu vực trống đã được khám phá.(-${staminaCost}sức mạnh thể chất)`, startsCombat: false }
  }
  ...
  const _handleOreTile = (tile: MineTile, staminaCost: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    // 仙缘能力：药山（shan_weng_2)Của tôi15%概率采到稀有草药
    ...
    return { success: true, message: `Tìm thấy nó${quantity}Một quặng!(-${staminaCost}sức mạnh thể chất)`, startsCombat: false }
  }
  ...
  const _handleMonsterTile = (tile: MineTile, staminaCost: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    combatMonsterHp.value = monster.hp
    combatRound.value = 0
    combatLog.value = [`Gặp phải${monster.name}!(HP: ${monster.hp})  (-${staminaCost}sức mạnh thể chất)`]
    combatIsBoss.value = false
    inCombat.value = true

    return { success: true, message: `Gặp phải${monster.name}!`, startsCombat: true }
  }
  ...
  const _handleBossTile = (tile: MineTile, staminaCost: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    const isFirstKill = !defeatedBosses.value.includes(monster.id)
    combatLog.value = [`BOSSChiến đấu! Gặp phải${monster.name}!(HP: ${monster.hp})${isFirstKill ? '' : '(phiên bản yếu đi)'}  (-${staminaCost}sức mạnh thể chất)`]
    combatIsBoss.value = true
    inCombat.value = true

    return { success: true, message: `BOSSLớp!${monster.name}Chặn đường!`, startsCombat: true }
  }
  ...
  const _handleStairsTile = (tile: MineTile, staminaCost: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    if (!stairsUsable.value) {
      ...
      if (floor?.specialType === 'infested') {
        ...
        return {
          success: true,
          message: `Đã tìm thấy cầu thang! Nhưng trước tiên bạn cần xóa những cái còn lại${remaining}Chỉ có quái vật mới có thể tiến lên.(-${staminaCost}sức mạnh thể chất)`,
          startsCombat: false
        }
      }
      if (floor?.specialType === 'boss') {
        return { success: true, message: `Đã tìm thấy cầu thang! Nhưng nó cần phải bị đánh bại trước tiênBOSSĐể tiến về phía trước.(-${staminaCost}sức mạnh thể chất)`, startsCombat: false }
      }
    }

    return { success: true, message: `Đã tìm thấy cầu thang! Bạn có thể đi đến cấp độ tiếp theo.(-${staminaCost}sức mạnh thể chất)`, startsCombat: false }
  }
  ...
  const _handleTrapTile = (tile: MineTile, staminaCost: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    if (playerStore.hp <= 0) {
      ...
      return { success: true, message: `Đã bước vào một cái bẫy! nhận được${damage}Điểm thiệt hại.${defeatResult.message}`, startsCombat: false }
    }

    return { success: true, message: `Đã bước vào một cái bẫy! nhận được${damage}Điểm thiệt hại.(-${staminaCost}sức mạnh thể chất)`, startsCombat: false }
  }
  ...
  const _handleTreasureTile = (tile: MineTile, staminaCost: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    let msg = 'Rương kho báu được tìm thấy!'
    if (items.length > 0) msg += `có${getRewardNames(items)}`
    if (money > 0) msg += `${items.length > 0 ? 'và' : 'có'}${money}văn bản`
    if (autoSoldMoney > 0) msg += `(Thiết bị trùng lặp được tự động bán+${autoSoldMoney}văn bản)`
    msg += `!(-${staminaCost}sức mạnh thể chất)`
  ...
  const _handleMushroomTile = (tile: MineTile, staminaCost: number): { success: boolean; message: string; startsCombat: boolean } => {
    ...
    tile.state = 'collected'
    return { success: true, message: `Đã sưu tầm${getRewardNames(items)}!(+3Thu thập kinh nghiệm, -${staminaCost}sức mạnh thể chất)`, startsCombat: false }
  }
  ...
  const useBombOnGrid = (bombId: string, centerIndex: number): { success: boolean; message: string } => {
    ...
    if (inCombat.value) return { success: false, message: 'Bom không thể được sử dụng trong khi chiến đấu.' }

    const bombDef = getBombById(bombId)
    if (!bombDef) return { success: false, message: 'Bom không hiệu quả.' }
    if (!inventoryStore.removeItem(bombId)) return { success: false, message: 'Quả bom không có trong ba lô.' }
    ...
    let msg = `${bombDef.name}Nó phát nổ!`
    if (oreCollected > 0) msg += `Đã sưu tầm${oreCollected}quặng`
    if (monstersKilled > 0) msg += `${oreCollected > 0 ? ',' : ''}bị đánh bại${monstersKilled}quái vật`
    if (oreCollected === 0 && monstersKilled === 0) msg += 'Đã mở một số khu vực'
    msg += '!'
    if (excavatorSaved) msg += '(Digger: Bom không bị tiêu hủy!)'
    return { success: true, message: msg }
  }

  // ==================== 进入 / 离开 ====================

  /** 进入Của tôi（可选择起始安全点楼lớp) */
  const enterMine = (startFromSafePoint?: number): string => {
    ...
    // BOSS lớp自动进入战斗（如果格子中有 boss 且入口邻格就是 boss)
    _checkAutoBossCombat()

    return `Tiến vào mỏ Yunyin, dòng chảy${currentFloor.value}lớp.`
  }

  /** 进入Mỏ sọ（可选择起始安全点楼lớp) */
  const enterSkullCavern = (startFromSafePoint?: number): string => {
    if (!isSkullCavernUnlocked()) return 'cần phải bị đánh bại trước tiên60lớpBOSSChỉ khi đó bạn mới có thể vào Skeleton Mine.'
    isExploring.value = true
    ...
    return `Bước vào Mỏ Skeleton, chương hiện tại${skullCavernFloor.value}lớp.`
  }

  /** 检查是否自动触发BOSS战（BOSS格在入口邻格时) */
  const _checkAutoBossCombat = () => {
    // BOSS lớp不自动触发——玩家需要自己探索到 BOSS 格
  }

  /** 获取所有已解锁的安全点（用于楼lớp选择) */
  const getUnlockedSafePoints = (): number[] => {
    const points: number[] = [0] // 0 = Từ lần đầu tiên1bắt đầu lớp
  ...
  const combatAction = (action: CombatAction): { message: string; combatOver: boolean; won: boolean } => {
    if (!inCombat.value || !combatMonster.value) {
      return { message: 'Không phải trong chiến đấu.', combatOver: true, won: false }
    }
    ...
    if (action === 'flee') {
      if (combatIsBoss.value) {
        combatLog.value.push('BOSSKhông có cách nào để thoát khỏi trận chiến!')
        return { message: 'BOSSKhông có cách nào để thoát khỏi trận chiến!', combatOver: false, won: false }
      }
      ...
      combatLog.value.push('Bạn đã bỏ chạy!')
      return { message: 'Thoát khỏi trận chiến thành công.', combatOver: true, won: false }
    }
    ...
    if (action === 'defend') {
      ...
      playerStore.takeDamage(damage)
      let defendMsg = `Bạn giơ khiên lên để phòng thủ và nhận${damage}Điểm thiệt hại.`

      // 守护者专精：防御回合khôi phục5HP
      if (skillStore.getSkill('combat').perk5 === 'defender') {
        playerStore.restoreHealth(5)
        defendMsg += '(Người giám hộ trả lời5HP)'
      }
      ...
      return { message: `Phòng thủ! nhận được${damage}Điểm thiệt hại.`, combatOver: false, won: false }
    }
    ...
    let msg = `bạn tấn công${monster.name}, gây ra${damageToMonster}Điểm thiệt hại.`
    if (isCrit) msg = `Đòn chí mạng!${msg}`

    // 匕首追加攻击（25%概率, gây ra50%伤害)
    let extraDamage = 0
    if (weaponDef?.type === 'dagger' && Math.random() < 0.25) {
      ...
      msg += ` Cuộc tấn công bổ sung! nguyên nhân phụ${bonusDamage}Một số thiệt hại!`
    }
    ...
    if (totalVampiric > 0) {
      ...
      if (healAmount > 0) {
        ...
        msg += ` Phục hồi ma cà rồng${healAmount}HP!`
      }
    }
    ...
    if (isStunned) {
      msg += ` ${monster.name}Tôi choáng váng!`
    ...
    if (skillStore.getSkill('combat').perk10 === 'acrobat' && Math.random() < 0.25) {
      msg += ` Bạn né khéo léo${monster.name}Phản công!`
      combatLog.value.push(msg)
      return { message: msg, combatOver: false, won: false }
    }
    ...
    msg += ` ${monster.name}Đánh trả và bạn nhận được${monsterDamage}Điểm thiệt hại.`
    combatLog.value.push(msg)
  ...
  const handleMonsterDefeat = (
        ...
        for (const wd of weaponDrops) {
          ...
          if (Math.random() < dropChance) {
            ...
            if (inventoryStore.hasWeaponExact(wd.weaponId, enchantId)) {
              ...
              msg += ` Vũ khí thu được:${displayName}!`
            }
          }
        }
      }
        ...
        for (const rd of ringDrops) {
          if (Math.random() < rd.chance + luckyBonus * rd.chance) {
            if (inventoryStore.hasRing(rd.ringId)) {
              ...
              msg += ` Đã nhận được chiếc nhẫn:${ringDef?.name ?? rd.ringId}!`
            }
          }
        }
      }
        ...
        for (const hd of hatDrops) {
          if (Math.random() < hd.chance + luckyBonus * hd.chance) {
            if (inventoryStore.hasHat(hd.hatId)) {
              ...
              msg += ` Đã có mũ:${hatDef?.name ?? hd.hatId}!`
            }
          }
        }
      }
        ...
        for (const sd of shoeDrops) {
          if (Math.random() < sd.chance + luckyBonus * sd.chance) {
            if (inventoryStore.hasShoe(sd.shoeId)) {
              ...
              msg += ` Đã có giày:${shoeDef?.name ?? sd.shoeId}!`
            }
          }
        }
      }
      if (monsterAutoSold > 0) msg += `(Thiết bị trùng lặp được tự động bán+${monsterAutoSold}văn bản)`
    }

    // BOSS 击败处理
    if (combatIsBoss.value) {
      if (isInSkullCavern.value) {
        // Mỏ sọBOSS：奖励铜钱và矿石（按深度缩放)
        const scFloor = skullCavernFloor.value
        const moneyReward = 200 + scFloor * 20
        playerStore.earnMoney(moneyReward)
        msg += ` lấy${moneyReward}Chữ!`
        ...
        msg += ` có${bonusOreCount}Một loại quặng hiếm!`
      } else {
        // 主Của tôiBOSS
        ...
        if (isFirstKill) {
          ...
          if (weaponId) {
            ...
            msg += ` thất bại đầu tiênBOSS! Nhận được vũ khí huyền thoại:${displayName}!`
          }
        }
        ...
        if (bossRingId && !inventoryStore.hasRing(bossRingId)) {
          ...
          msg += ` Đã nhận được chiếc nhẫn:${bossRingDef?.name ?? bossRingId}!`
        }
        ...
        if (bossHatId && !inventoryStore.hasHat(bossHatId)) {
          ...
          msg += ` Đã có mũ:${bossHatDef?.name ?? bossHatId}!`
        }
        ...
        if (bossShoeId && !inventoryStore.hasShoe(bossShoeId)) {
          ...
          msg += ` Đã có giày:${bossShoeDef?.name ?? bossShoeId}!`
        }
        ...
        if (oreRewards) {
          ...
          msg += ` có${getRewardNames(oreRewards)}!`
        }
      }
    }

    msg += ` ${monster.name}Bị đánh bại!(+${monster.expReward}kinh nghiệm)`
    if (drops.length > 0) msg += ` Vật phẩm bị rơi.`
    combatLog.value.push(msg)
    ...
    if (monstersDefeatedCount.value >= totalMonstersOnFloor.value && totalMonstersOnFloor.value > 0) {
      ...
      if (floor?.specialType === 'infested') {
        ...
        playerStore.earnMoney(clearRewards.money)
        msg += ` Lớp lây nhiễm đã được loại bỏ! nhận được${getRewardNames(clearRewards.items)}và${clearRewards.money}Chữ!`
      }
    } else if (floor?.specialType === 'infested') {
      const remaining = totalMonstersOnFloor.value - monstersDefeatedCount.value
      msg += ` trái${remaining}Một con quái vật!`
    }
  ...
  const handleDefeat = (): { message: string; combatOver: boolean; won: boolean } => {
    ...
    const location = wasInSkullCavern ? 'Mỏ sọ' : 'Của tôi'
    const parts: string[] = [`bạn là${location}rơi xuống……`]
    parts.push('Mất một nửa chiến lợi phẩm')
    if (droppedItems.length > 0) parts.push(`và${droppedItems.length}mặt hàng ba lô`)
    if (moneyLost > 0) parts.push(`và${moneyLost}văn bản`)
    parts.push(', đã được gửi trở lại lối vào.')
  ...
  /** Chuyển tiếp tới下一lớp */
  const goNextFloor = (): { success: boolean; message: string } => {
    if (!isExploring.value) return { success: false, message: 'Bạn không ở trong hầm mỏ.' }
    if (!stairsFound.value) {
      return { success: false, message: 'Bạn vẫn chưa tìm thấy cầu thang nên hãy tiếp tục khám phá.' }
    }
    if (!stairsUsable.value) {
      const floor = getActiveFloorData()
      if (floor?.specialType === 'infested') {
        const remaining = totalMonstersOnFloor.value - monstersDefeatedCount.value
        return { success: false, message: `Ngoài ra${remaining}Chỉ có quái vật chưa bị tiêu diệt và không thể tiến về phía trước!` }
      }
      if (floor?.specialType === 'boss') {
        return { success: false, message: 'phải bị đánh bạiBOSSChỉ khi đó chúng ta mới có thể tiến về phía trước!' }
      }
      return { success: false, message: 'Cầu thang tạm thời không có sẵn.' }
    }
    ...
    if (isInSkullCavern.value) {
      ...
      if (currentFloor.value >= MAX_MINE_FLOOR) {
        ...
        if (isSkullCavernUnlocked()) {
          ...
          cacheSkullFloor(1)
          _generateGrid()
          return { success: true, message: 'Bạn đã đi qua vết nứt sâu nhất trong mỏ và tiến vào Mỏ Skeleton.1Lớp!' }
        }
        return { success: false, message: 'Chúng ta đã tới phần sâu nhất của mỏ! (đánh bại60lớpBOSSMỏ xương có thể được mở khóa)' }
      }

      currentFloor.value++
      useAchievementStore().recordMineFloor(currentFloor.value)

      // 到达新的安全点时保存（只在到达更高lớp时更新,避免电梯返回低lớp后覆盖进度)
    ...
    const locationName = isInSkullCavern.value ? 'Mỏ sọ' : ''
    const specialLabels: Record<string, string> = {
      mushroom: 'Hang Nấm',
      treasure: 'Cấp độ rương kho báu',
      infested: 'lớp lây nhiễm',
      dark: 'tầng sông ngầm',
      boss: 'BOSSlớp'
    }
    const specialLabel = newFloor?.specialType ? (specialLabels[newFloor.specialType] ?? '') : ''
    let msg = `Chuyển tiếp tới${locationName}Không.${activeFloorNum}lớp.${newFloor?.isSafePoint ? '(Hãy an toàn!)' : ''}`
  ...
  const leaveMine = (): string => {
    ...
    if (isInSkullCavern.value) {
      isInSkullCavern.value = false
      cachedSkullFloorData.value = null
      return 'Bạn đã rời khỏi Skull Mine.'
    }
    return 'Bạn đã rời khỏi mỏ.'
  }
  ...
  const useCombatItem = (itemId: string, quantity: number = 1): { success: boolean; message: string } => {
    if (!inCombat.value && !isExploring.value) return { success: false, message: 'Không có trong hầm mỏ.' }

    // 公会徽章：永久+3攻击力
    if (itemId === 'guild_badge') {
      const actual = Math.min(quantity, inventoryStore.getItemCount('guild_badge'))
      if (actual <= 0) return { success: false, message: 'Không có huy hiệu bang hội.' }
      inventoryStore.removeItem('guild_badge', actual)
      guildBadgeBonusAttack.value += 3 * actual
      const msg = `Huy hiệu bang hội đã qua sử dụng×${actual}, sức tấn công là vĩnh viễn+${3 * actual}!`
    ...
    if (itemId === 'life_talisman') {
      ...
      if (actual <= 0) return { success: false, message: 'Không có bùa sống.' }
      inventoryStore.removeItem('life_talisman', actual)
      guildBonusMaxHp.value += 15 * actual
      const msg = `Bùa sống đã qua sử dụng×${actual}, giá trị sức khỏe tối đa là vĩnh viễn+${15 * actual}!`
      if (inCombat.value) combatLog.value.push(msg)
      return { success: true, message: msg }
    }
    ...
    if (itemId === 'lucky_coin') {
      ...
      const msg = `Đồng xu may mắn đã qua sử dụng×${actual}, tỷ lệ rớt quái vật là vĩnh viễn+${5 * actual}%!`
      if (inCombat.value) combatLog.value.push(msg)
      return { success: true, message: msg }
    }
    ...
    if (itemId === 'defense_charm') {
      ...
      if (actual <= 0) return { success: false, message: 'Không có bùa hộ mệnh.' }
      inventoryStore.removeItem('defense_charm', actual)
      guildBonusDefense.value += 0.03 * actual
      const msg = `Đã sử dụng một lá bùa×${actual}, phòng thủ là vĩnh viễn+${3 * actual}%!`
      if (inCombat.value) combatLog.value.push(msg)
      return { success: true, message: msg }
    }
    ...
    if (itemId === 'slayer_charm') {
      if (slayerCharmActive.value) return { success: false, message: 'Hiệu ứng Bùa Săn Qu�� được kích hoạt.' }
      if (!inventoryStore.removeItem('slayer_charm')) return { success: false, message: 'Không có bùa săn quỷ.' }
      slayerCharmActive.value = true
      const msg = 'Bùa săn quái vật được sử dụng và tỷ lệ rơi ra quái vật trong chuyến thám hiểm này là+20%!'
      if (inCombat.value) combatLog.value.push(msg)
      return { success: true, message: msg }
    }
    ...
    if (!def) return { success: false, message: 'Mục không xác định.' }
    ...
    if (itemId.startsWith('food_')) {
      ...
      if (hpFull && staminaFull) {
        return { success: false, message: 'Cả thể lực lẫn sức khỏe đều đầy đủ.' }
      }
      // 查找背包中该食物的最低品质
    ...
    if (hasHpRestore && !hasStaminaRestore && hpFull) {
      return { success: false, message: 'Điểm sinh mệnh đã đầy.' }
    }
    if (hasStaminaRestore && !hasHpRestore && staminaFull) {
      return { success: false, message: 'Thể lực đã đầy đủ.' }
    }
    if (hpFull && staminaFull && (hasHpRestore || hasStaminaRestore)) {
      return { success: false, message: 'Cả thể lực lẫn sức khỏe đều đầy đủ.' }
    }

    if (!inventoryStore.removeItem(itemId)) return { success: false, message: `Không${def.name}.` }

    // 炼金师专精：食物khôi phục+50%
    ...
    if (hasHpRestore) {
      ...
      parts.push(`khôi phục${def.healthRestore! >= 999 ? 'Tất cả' : restore}HP`)
    }
    if (hasStaminaRestore) {
      const restore = Math.floor(def.staminaRestore! * alchemistBonus)
      playerStore.restoreStamina(restore)
      parts.push(`khôi phục${restore}sức mạnh thể chất`)
    }

    const msg = `đã sử dụng${def.name},${parts.join('và')}!`
    if (inCombat.value) combatLog.value.push(msg)
    return { success: true, message: msg }
  }

  /** 在探索中使用怪物诱饵（本lớp怪物数量翻倍) */
  const useMonsterLure = (): { success: boolean; message: string } => {
    if (!isExploring.value) return { success: false, message: 'Không có trong hầm mỏ.' }
    if (inCombat.value) return { success: false, message: 'Mồi quái vật không thể được sử dụng trong khi chiến đấu.' }
    if (!inventoryStore.removeItem('monster_lure')) return { success: false, message: 'Không có mồi quái vật.' }

    const floor = getActiveFloorData()
    if (!floor) return { success: true, message: 'Mồi quái vật được sử dụng, nhưng lớp này không hiệu quả.' }
    ...
    if (monstersToAdd === 0) {
      return { success: true, message: 'Mồi quái vật đã được sử dụng nhưng không còn chỗ cho thêm quái vật ở tầng này.' }
    }
    ...
    return { success: true, message: `Mồi quái vật đã được sử dụng! Lớp này đã được thêm vào${monstersToAdd}Một con quái vật.` }
  }
