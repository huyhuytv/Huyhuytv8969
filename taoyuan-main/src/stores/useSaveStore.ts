  ...
  const exportSave = (slot: number): boolean => {
    try {
      ...
      const name = info?.exists
        ? `Thị trấn Đào Viên_Lưu trữ${slot + 1}_Không.${info.year}năm${SEASON_NAMES[info.season as keyof typeof SEASON_NAMES] ?? info.season}Không.${info.day}ngày`
        : tText(`Thị trấn Đào Viên_Lưu trữ${slot + 1}`, `Thị trấn Đào Viên_Archive${slot + 1}`)
