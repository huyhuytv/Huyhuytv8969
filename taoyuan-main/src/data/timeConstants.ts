...
export const DAY_END_HOUR = 26 // sáng sớm2điểm
...
export const WEEKDAY_NAMES: Record<Weekday, string> = {
  get mon() { return i18n.global.t('time.week_mon') },
  get tue() { return i18n.global.t('time.week_tue') },
  get wed() { return i18n.global.t('time.week_wed') },
  get thu() { return i18n.global.t('time.week_thu') },
  get fri() { return i18n.global.t('time.week_fri') },
  get sat() { return i18n.global.t('time.week_sat') },
  get sun() { return i18n.global.t('time.week_sun') }
}
...
export const formatHour = (hour: number): string => {
  ...
  if (hour >= 6 && hour < 12) return i18n.global.t('time.time_format', { period: i18n.global.t('time.am'), h: realHour, m: '00' })
  if (hour === 12) return i18n.global.t('time.time_format', { period: i18n.global.t('time.noon'), h: 12, m: '00' })
  if (hour > 12 && hour < 18) return i18n.global.t('time.time_format', { period: i18n.global.t('time.pm'), h: realHour, m: '00' })
  if (hour >= 18 && hour < 24) return i18n.global.t('time.time_format', { period: i18n.global.t('time.evening'), h: realHour, m: '00' })
  return i18n.global.t('time.time_format', { period: i18n.global.t('time.midnight'), h: realHour, m: '00' })
}
...
export const formatTime = (hour: number): string => {
  ...
  if (h >= 6 && h < 12) return i18n.global.t('time.time_format', { period: i18n.global.t('time.am'), h: realH, m: mm })
  if (h === 12 && m === 0) return i18n.global.t('time.time_format', { period: i18n.global.t('time.noon'), h: 12, m: mm })
  if (h >= 12 && h < 18) return i18n.global.t('time.time_format', { period: i18n.global.t('time.pm'), h: realH, m: mm })
  if (h >= 18 && h < 24) return i18n.global.t('time.time_format', { period: i18n.global.t('time.evening'), h: realH, m: mm })
  return i18n.global.t('time.time_format', { period: i18n.global.t('time.midnight'), h: realH, m: mm })
}
...
const LOCATION_GROUP_NAMES: Record<LocationGroup, string> = {
  get farm() { return i18n.global.t('time.loc_farm') },
  get village_area() { return i18n.global.t('time.loc_village') },
  get nature() { return i18n.global.t('time.loc_nature') },
  get mine() { return i18n.global.t('time.loc_mine') },
  get hanhai() { return i18n.global.t('time.loc_hanhai') }
}
...
export const SHOP_SCHEDULES: ShopSchedule[] = [
  { tabKey: 'shop', name: 'Khu thương mại Đào Nguyên', closedDays: [], openHour: 6, closeHour: 24 },
  { tabKey: 'upgrade', name: 'Xưởng chế tác', closedDays: ['sun'], openHour: 8, closeHour: 20 }
]
...
export const isShopOpen = (tabKey: string, day: number, hour: number): { open: boolean; reason?: string } => {
  ...
  if (schedule.closedDays.includes(weekday)) {
    return { open: false, reason: `${schedule.name}Hôm nay (${WEEKDAY_NAMES[weekday]}) nghỉ ngơi.` }
  }
  if (hour < schedule.openHour) {
    return { open: false, reason: `${schedule.name}Chưa mở (${formatHour(schedule.openHour)}mở cửa).` }
  }
  if (hour >= schedule.closeHour) {
    return { open: false, reason: `${schedule.name}Đã đóng cửa rồi (${formatHour(schedule.closeHour)}đóng cửa lại).` }
  }
  return { open: true }
}
...
export const getNpcUnavailableReason = (npcId: string, day: number, hour: number, season?: Season): string | null => {
  ...
  if (!isBday && schedule.availableDays !== 'all' && !schedule.availableDays.includes(weekday)) {
    return 'Hôm nay không có ở trong thôn'
  }
  if (hour < schedule.availableHours.from) return 'Chưa ra ngoài'
...
// === 深夜惩罚 ===
/** 渐进式晚睡恢复：根据就寝时间线性递减 */
export const LATE_NIGHT_RECOVERY_MAX = 0.9 // 24phục hồi trước khi đi ngủ90%
export const LATE_NIGHT_RECOVERY_MIN = 0.6 // 25phục hồi trước khi đi ngủ60%
